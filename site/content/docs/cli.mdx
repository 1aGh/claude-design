...
`mdcc` is intentionally small. It does mechanical scaffolding and config; the *interesting* setup work happens inside Claude Code via `/flow:init`, which calls `mdcc init` as step one.
