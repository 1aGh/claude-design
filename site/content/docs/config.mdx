...
Auto-detected by `/flow:setup-onboard` and refreshed by `/flow:setup-codebase-map`. Used for routing decisions — which test runner, whether to gate on workspace builds.
...
The **release runbook** (`releaseGuide`) is plain Markdown — `H2` sections become steps, `bash` blocks become candidate commands (run only after user confirmation). Scaffolded with a provider-appropriate stub during `/flow:setup-onboard`.
