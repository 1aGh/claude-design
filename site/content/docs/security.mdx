...
Once linked, the hub is **semi-trusted**: hub-pushed content lands on your disk verbatim, the same posture as `git pull` from a remote branch ([DDR-054](https://github.com/1aGh/maude/blob/main/.ai/archive/decisions/DDR-054-linked-mode-trust-model-and-task-4-hardening.md)). Four mechanisms bound what that buys an attacker:
...
Because the canvas format is `.tsx`-only ([DDR-060](https://github.com/1aGh/maude/blob/main/.ai/archive/decisions/DDR-060-tsx-only-format-breaks-html-centric-sync.md)), a typical linked project syncs **nothing** until you opt a specific canvas in — and `maude design status` says so loudly. See [Linking peers](/docs/hub/linking#what-syncs-tsx-canvases-by-per-canvas-opt-in) for the operational detail.
...
- **What's in git vs. per-machine.** A managed, idempotent `.gitignore` block separates committed source from regenerable per-machine runtime state ([DDR-056](https://github.com/1aGh/maude/blob/main/.ai/archive/decisions/DDR-056-linked-mode-gitignore-strategy.md)). Your token lives outside the repo and is never in it.
