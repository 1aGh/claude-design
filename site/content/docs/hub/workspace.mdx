...
| --- | --- |
| `HUB_DASHBOARD_URL` | The address the "← Dashboard" link points at. Omit it and the link is not rendered at all. |
| `MAUDE_REPORT_URL` | Where the Report-a-Bug dialog sends its (consented, scrubbed) bundle. Omit it and reports go to Maude's own endpoint — nothing is ever sent without the preview and your click. |
...
Exports that need rendering (PNG, PDF, PPTX, video) therefore don't happen *in* the workspace: they run in Maude Desktop on your machine, or — if you deploy it — in the dedicated **`maude-render` sidecar**, a separate container that holds the browser and none of the workspace's secrets ([Self-hosting → the render sidecar](/docs/hub/self-host)). Without either, the export dialog says so instead of failing; ZIP export always works, because zipping evaluates nothing.
