...
- **WSS is mandatory.** The hub refuses to boot over plaintext `http://` to a non-loopback host. TLS terminates at your proxy (Fly / Caddy / ALB / Cloudflare / Tailscale), so the hub itself sees `http://` internally — `HUB_PUBLIC_URL` must declare the `https://` URL. Local dev sets `HUB_INSECURE_HTTP=1` to opt out.
