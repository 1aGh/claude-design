...
The hub is a pre-configured [Hocuspocus](https://tiptap.dev/docs/hocuspocus) server (production-tested, MIT, Node-native — [DDR-052](https://github.com/1aGh/maude/blob/main/.ai/archive/decisions/DDR-052-hocuspocus-over-partykit-for-hub.md) explains why not PartyKit) with token auth and SQLite persistence. You never touch Yjs.
...
AWS has its own page — [On AWS](/docs/hub/aws) — because two of its defaults
(Fargate's ephemeral storage, EFS for the data volume) quietly break a
workspace, and the fix is specific enough to be worth spelling out once rather
than half-explaining here.
