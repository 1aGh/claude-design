...
> Sub-path serving landed **after v0.24.0**. The published `ghcr.io/1agh/maude-hub:latest` only picks it up on the next release — if you need it before then, build the image from source: `docker build -t maude-hub plugins/design/hub`.
...
For local contributor testing without a cloud account, see [`plugins/design/hub/CONTRIBUTING.md`](https://github.com/1aGh/maude/blob/main/plugins/design/hub/CONTRIBUTING.md).
