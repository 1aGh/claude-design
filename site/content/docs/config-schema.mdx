...
In-product "What's New" feed. When enabled, /flow:done offers to append a user-facing entry for the closed-out feature to `feed`, delegating to the named project skill if set. Generic + opt-in: absent or `enabled:false` → /flow:done skips it silently. Maude wires this to plugins/design/dev-server/whats-new.json, surfaced in the canvas browser.
