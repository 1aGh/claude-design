...
Assumed: pnpm workspace, possibly Turbo or Nx on top. Multiple deployable apps + shared packages.
...
The dev server resolves the design root via `--root` arg → `$CLAUDE_PROJECT_DIR` → `cwd`. From any package directory, set:
...
- **One `CLAUDE.md` at root + per-package overrides.** The root `CLAUDE.md` covers cross-cutting concerns (commit conventions, branch model). Per-package `CLAUDE.md` (placed in `apps/web/`, `apps/api/`, etc.) override or extend it.
...
- **PR scopes.** Conventional commits for monorepos benefit from a scope: `feat(web): …`, `fix(ui): …`. `/flow:done` formats commits to match `conventions.commits` but won't auto-pick the scope — you can add it when prompted.
