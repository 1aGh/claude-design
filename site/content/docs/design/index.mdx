...
| **Layers** | A browsable tree of the artboard's elements with type icons — including group rows for wrapper `<div>`s, so nothing nests at the wrong depth. Click to select, hover to highlight, toggle any layer's visibility from its row, and the tree auto-reveals whatever you select on the canvas. |
