...
Eight daily verbs (no prefix), three setup verbs (`setup-*` group), and one validator (`smoke`). The bare `/design` form was a v0.8 one-version compat stub redirecting to `/design:edit`. **Removed in v0.9**.
...
Per-repo behavior lives at `.design/config.json` (handoff targets, opt-out scopes per canvas, etc.). The schema is informal. See [`plugins/design/DESIGN_CONFIG.md`](https://github.com/1aGh/maude/blob/main/plugins/design/DESIGN_CONFIG.md) when it lands (deferred to a later phase).
