...
| `_active.json` | `{ active, open_tabs, selected, last_change }`. The canvas-shell selection layer pushes the user's currently-selected element here over WebSocket so `/design:edit "<feedback>"` knows what to scope to. |
