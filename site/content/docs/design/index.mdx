...
A zero-dep Node dev server injects an inspector overlay. **Cmd+Click any element** to scope the next `/design:edit "<feedback>"` to that selection. The server tracks active tab + selected element via WebSocket and writes `<designRoot>/_active.json` for the slash commands to read.
