...
3. Set frontmatter: `name: <verb-or-group-verb>` (bare slug, **no** `design:` prefix — Claude Code namespaces it itself at registration; baking the prefix in by hand doubles it into `/design:design:<verb>`, per DDR-191, superseding DDR-006), `category: <group>`, `description: <one-liner>`, `argument-hint:` if any.
