...
description: Slash-command groups for the design plugin — daily + setup. Mirrors plugins/design/CATEGORIES.md.
---

The design plugin organizes its slash commands into two groups: `daily` (verb-as-complete-action, called every iteration cycle) and `setup-*` (one-shot bootstrapping operations).

Mirrors the flow plugin's [categorization](https://github.com/1aGh/md-claude/blob/main/plugins/flow/CATEGORIES.md) pattern. Claude Code doesn't support slash-command subdirectory namespacing — the `<group>-<verb>` prefix is the working substitute. Typing `/design:setup-` autocompletes only the three setup verbs.
...
The auto-generated reference pages under `/docs/reference/design/` are produced by `site/scripts/build-command-reference.mjs` from each command file's frontmatter on every `pnpm prebuild`.
