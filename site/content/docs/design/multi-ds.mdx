...
The skill detects `additional-ds` mode (config exists, `marketing` not in `designSystems[]`), runs the same staged discovery (vision → research → direction pick → refinement) plus a `Q_purpose` prompt, offers an inheritance picker before the direction gate (typography, voice, iconography, motion. Multi-select; inherited fields render identically across all direction tiles), and scaffolds `system/marketing/` next to the existing DS.
...
No production users had multi-DS when the layout was introduced, so no migration tooling ships.
