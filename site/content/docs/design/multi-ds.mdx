...
The skill detects `additional-ds` mode (config exists, `marketing` not in `designSystems[]`), runs the same 3-stage discovery plus a `Q_purpose` prompt, offers an inheritance picker (typography, voice, iconography, motion. Multi-select), and scaffolds `system/marketing/` next to the existing DS.
