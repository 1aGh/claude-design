...
1. **Onboard runs first** (if `.design/config.json` is missing). `/design:setup-onboard` does dependency pre-flight (Node ≥ 20, git), writes a skeleton config with empty `designSystems: []`, and prints install hints for soft deps (agent-browser, mdcc, CLAUDE.md / `.ai/` recommendations).
