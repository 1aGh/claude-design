...
2. **Skill `design-system` loads in bootstrap mode.** It detects `first-bootstrap` (no DS yet) and runs **staged discovery** (DDR-033 + DDR-147): **Stage 1 — Vision** (free-text prose prompts about product, audience, mood, and voice — no brand-name option ladders to bias you), **Stage 2 — Research** (the `ux-research-agent` builds a domain reference pool from the web and returns per-decision recommendations with confidence), **Stage 3 — Direction gate** (the moodboard: by default ~3 genuinely distinct **direction tiles** — real type in context, a 60/30/10 palette strip, the signature treatment, a voice line, each with a one-line thesis — composed from the research payload and rendered side by side; you pick one on gut feel, or mix a base direction with a named element from another), **Stage 4 — Refinement** (a short `AskUserQuestion` round only for what the pick didn't settle — typically 1–3 questions).
3. **LOCK gate.** One echo of the picked direction + your refinement answers, then the moodboard freezes as the direction contract the generation consumes verbatim. The moodboard stays as a normal canvas you can revisit and comment on. Nobody knows what font they want until they see it used — so you *see* directions before answering any refinement question.
...
After the Core scaffold, open Claude Code and run `/design:setup-ds project "<one-line product brief>" --force` to re-run with the full staged discovery (vision → research → direction pick → refinement). You'll get a richer, project-flavored result.
...
The skill detects `additional-ds` mode (config exists, `marketing` not in `designSystems[]`), runs the same staged discovery plus a `Q_purpose` prompt up front and an inheritance picker between research and the direction gate (inherited fonts/motion render identically across all direction tiles, so a pick can never override them), and scaffolds `system/marketing/` alongside the existing DS. Subsequent `/design:new --ds=marketing "<Name>" "<brief>"` creates canvases scoped to that DS.
...
The skill detects `re-bootstrap` mode, lossy-infers Stage 1 (Vision) from the existing DS (README + tokens + layout), and you confirm or correct it in one pass; Stage 2 (Research) always re-runs, and the direction gate shows a single tile (the inferred vision is too low-confidence to seed divergent directions). Changed answers re-generate the affected files. Without `--force`, the command refuses with a list of safer alternatives (`/design:edit` for incremental, `/design:setup-ds <new-name>` for a sibling DS).
