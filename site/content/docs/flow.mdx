...
The `flow` plugin's commands follow a strict naming convention: **daily** verbs called every feature cycle stay terse (`plan`, `execute`, `done`, `validate`, `release`). Everything else uses `<group>-<verb>` (e.g. `setup-onboard`, `bug-rca`, `record-ddr`).
...
| Command | What it does |
| ------- | ------------ |
| `/flow:setup-onboard` | Scaffold `.ai/`, auto-detect stack, populate `workflows.config.json`, defer to `/init` for `CLAUDE.md`. |
