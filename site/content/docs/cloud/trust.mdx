...
| Export before teardown | `apps/cloud/reconcile.mjs` — `purged` is reachable only via `exported` |
| Suspension retains data | The same state machine (`cli/lib/cell-plan.mjs`) |
| One project cannot read another | Per-project container; R2 keys are prefix-isolated per tenant |
