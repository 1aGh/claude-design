...
Per-repo configuration consumed by the flow plugin's commands, skills, and subagents. Place at &lt;repo>/.ai/workflows.config.json. Missing file = sensible defaults; missing keys = same.
...
Auto-detected technology stack snapshot. Populated by /flow:setup-onboard, refreshed by /flow:setup-codebase-map. Flow commands read this for routing decisions (e.g. which test runner to invoke, whether to gate on monorepo workspace builds). Use `unknown` when detection fails — never invent a value.
...
Team conventions that flow commands enforce or display. Auto-detected where possible (e.g. `commits` from commitlint.config.*); otherwise asked once in /flow:setup-onboard. Document non-obvious 'why' in `.ai/decisions/` as DDRs.
...
Path (repo-relative) to the project-owned release runbook that /flow:release walks step-by-step. The runbook is plain Markdown — H2 sections become steps, ```bash``` blocks become candidate commands (run only after user confirmation). Scaffolded with a provider-appropriate stub during /flow:setup-onboard.
