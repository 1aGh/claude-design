// design-canvas-viewport.tsx — ported from
// plugins/design/dev-server/runtime/design-canvas.jsx (Phase 3.6.x).
//
// Project-owned canvas viewport: pan/zoom, persisted artboard reorder + labels,
// fullscreen focus mode, lazy-mount via IntersectionObserver. Imported via
// @mdcc/canvas-lib so UI canvases (Docs Site, Canvas Viewport) get the full
// pre-migration experience.
//
// React API is named-imported; original used the legacy babel-runtime
// window-global `React` namespace. Behaviour unchanged.
...
export function DesignCanvas({ children, minScale, maxScale, style }) {
  const [state, setState] = useState({ sections: {}, focus: null, viewport: null });
  ...
  Children.forEach(children, (sec) => {
    ...
    const srcIds = [];
    Children.forEach(sec.props.children, (ab) => {
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {}, initialViewport, onViewportChange }) {
  ...
  const cachedRect = useRef(null);  // cached vp bounds; invalidated on resize / settle
