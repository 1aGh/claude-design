...
import { Caption, AgentCursor, typewriter } from '../../../lib/maude-stage';
...
export const DesignNewScene = () => {
  ...
  const curOpacity = interpolate(frame, [105, 125], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  ...
  return (
    <AbsoluteFill style={{ background: t.bg0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '50%', background: t.bg0, padding: '70px 64px', borderRight: `2px solid ${t.accentMuted}` }}>
        ...
        {lines.map((l) => (
          <div key={l.text} style={{ fontFamily: maude.font.mono, fontSize: 24, lineHeight: 1.9, color: l.color, ...appear(l.at, 10) }}>
            {l.text}
          </div>
        ))}
      </div>
      ...
      >
        {/* artboard */}
        <div style={{ width: 720, background: t.bg1, border: `1px solid ${t.border}`, borderRadius: 16, boxShadow: '0 24px 70px rgba(0,0,0,0.45)', overflow: 'hidden', ...appear(70, 30) }}>
          {/* title bar */}
          <div style={{ display: 'flex', gap: 7, alignItems: 'center', padding: '16px 22px', borderBottom: `1px solid ${t.borderSubtle}`, ...appear(84) }}>
          ...
          <div style={{ padding: 40, display: 'flex', flexDirection: 'column', gap: 22 }}>
            <div style={{ fontFamily: maude.font.display, fontWeight: 700, fontSize: 56, color: t.fg0, lineHeight: 1.05, ...appear(112) }}>
              Recipe Recap
            </div>
            <div style={sk('70%', 142)} />
            <div style={sk('88%', 168)} />
            ...
            <div style={{ display: 'flex', gap: 14, marginTop: 10, ...appear(214) }}>
              <span style={{ background: t.accent, color: t.accentFg, fontFamily: maude.font.body, fontWeight: 600, fontSize: 22, padding: '12px 26px', borderRadius: 10 }}>Scale portions</span>
              <span style={{ border: `1px solid ${t.border}`, color: t.fg1, fontFamily: maude.font.body, fontSize: 22, padding: '12px 26px', borderRadius: 10 }}>Print</span>
            </div>
          </div>
        </div>
      </div>
