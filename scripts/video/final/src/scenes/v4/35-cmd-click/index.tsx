  ...
  return (
    <AbsoluteFill>
      <DottedCanvas theme="dark" style={{ alignItems: 'center', justifyContent: 'center' }}>
        {/* artboard */}
        <div style={{ width: 860, background: t.bg1, border: `1px solid ${t.border}`, borderRadius: 18, boxShadow: '0 24px 70px rgba(0,0,0,0.45)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: 48, display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ fontFamily: maude.font.display, fontWeight: 700, fontSize: 60, color: t.fg0 }}>Recipe Recap</div>
            ...
            <div style={{ position: 'relative', alignSelf: 'flex-start', marginTop: 8 }}>
              <span style={{ display: 'inline-block', background: t.accent, color: t.accentFg, fontFamily: maude.font.body, fontWeight: 600, fontSize: 24, padding: '14px 30px', borderRadius: 12 }}>
                Scale portions
              </span>
              {/* halo ring */}
              <span
        ...
        <div style={{ position: 'absolute', left: curX, top: curY }}>
          <svg width={34} height={34} viewBox="0 0 16 16" fill={t.presence} aria-hidden><path d="M3 2l9 4.4-4 1.1-1.1 4z" /></svg>
          <span style={{ position: 'absolute', left: 24, top: 22, fontFamily: maude.font.mono, fontSize: 16, fontWeight: 600, color: '#fff', background: t.accent, borderRadius: 6, padding: '2px 8px' }}>⌘ click</span>
        </div>

        <Caption theme="dark" frame={frame} from={120} text="cmd+click. the exact file Claude needs." />
      </DottedCanvas>
    </AbsoluteFill>
  );
};
