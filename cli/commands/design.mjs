...
function ensureBunOnPath(childEnv, pkgRoot) {
  if (bunOnPath(childEnv.PATH)) return childEnv;
  const bunnable = resolveBunnable(pkgRoot);
  if (!bunnable) return childEnv; // helper's own `command -v bun` guard reports the miss
  try {
    const shimDir = join(homedir() || tmpdir(), '.cache', 'maude', 'bun-shim');
    mkdirSync(shimDir, { recursive: true, mode: 0o700 });
    const shim = join(shimDir, 'bun');
    writeFileSync(
      shim,
      `#!/bin/sh\n# maude bun shim (RCA G3) — the bundled compiled binary IS Bun;\n# BUN_BE_BUN=1 makes it act as the \`bun\` CLI so design helpers run with no\n# user-installed bun. Regenerated by cli/commands/design.mjs.\nBUN_BE_BUN=1 exec ${JSON.stringify(bunnable)} "$@"\n`,
      { mode: 0o755 }
    );
    chmodSync(shim, 0o755);
    return { ...childEnv, PATH: `${shimDir}:${childEnv.PATH || ''}` };
  } catch {
    return childEnv; // best-effort; the helper reports the miss with an install hint
  }
}
...
function runBinDispatch(verb, { args, pkgRoot }) {
  ...
  // RCA G3 — make `bun` resolvable to the helper from the bundled compiled
  // binary when no standalone `bun` is installed (desktop / npm). No-op in dev.
  const finalEnv = ensureBunOnPath(childEnv, pkgRoot);
