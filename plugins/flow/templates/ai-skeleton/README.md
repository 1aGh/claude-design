...
| `decisions/` | `/flow:ddr` | Design Decision Records. Append-only, numbered, never deleted. |
| `reviews/` | `/flow:code-review`, `/flow:retro` | Code review snapshots and retrospective notes from completed work. |
...
| `logs/` | `/flow:validate`, `/flow:ai-health` | System-level audit logs (validation runs, infrastructure reviews). |
...
| `context/` | `/flow:map-codebase`, `/flow:context` | Living snapshots of codebase architecture (auto-generated, refresh periodically). |
...
Run `mdcc init` from your repo root to scaffold this skeleton (idempotent — never overwrites). Then fill in `PROJECT.md` (rendered to repo root) and `.ai/<name>-prd.md`.
