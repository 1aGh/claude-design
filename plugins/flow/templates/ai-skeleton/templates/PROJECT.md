# Project

> Machine-readable project identity. Commands read this file for auto-configuration. Generated on bootstrap (via `mdcc init` or `/flow:onboard`), then maintained manually.

## Identity

- name: <PROJECT_NAME>
- org: TODO
- repo: TODO
- description: TODO. See `.ai/<project>-prd.md` if present.

## Tracking

- system: TODO (github | linear | jira | none)
- project: TODO

## Stack

> Fill in once the first code exists. Pre-implementation, leave as TODO.

- language: TODO
- framework: TODO
- build: TODO
- package-manager: TODO
- monorepo: TODO
- css: TODO
- router: TODO
- testing: TODO

## Constraints

- branching: TODO (github-flow | trunk-based | gitflow)
- commits: conventional
- prohibited: TODO — list project-specific design or code constraints (see `.ai/<project>-design-system.md` if present)

## Local Layout

> Per-developer paths. Do not commit real values — keep them in `PROJECT.local.md` (untracked).

- workspace-root: TODO
- env-file: TODO
