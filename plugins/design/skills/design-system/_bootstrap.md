...
- Each tile's imagery = its cluster's slice of the seed `reference_images[]` (matched via `anchor → reference_products[].mood_tag`) + the always-render CSS/SVG scrap layer. **Download-first (DDR-147 addendum):** at compose time, fetch every used image into `<designRoot>/assets/moodboard/<ds>/` and reference the LOCAL path in the canvas — hotlink an external URL only when the download fails, and keep the per-`<img>` `onError` fallback as the last resort. Local files render deterministically (user's browser, headless screenshots, offline, split-origin CSP `img-src 'self'`) and keep the persistent canvas (DDR-136) actually persistent — hotlinks rot. **Verify CONTENT, not just filetype: Read (visually inspect) every downloaded image before composing** — size/type checks pass decorative junk (noise textures, shadow gradients harvested off marketing pages), and a junk asset in an anchor slot kills the tile's mood (live dogfood 2026-07-03). Zero live images is still a valid tile — the scrap layer + provenance backbone carry it.
  ...
  > **⚠ Untrusted-fetch rails (security — mandatory, DDR-147 addendum).** Every `reference_images[].url` is **attacker-influenceable** (it came from a web page the research pass ingested) and `assets/moodboard/<ds>/` is **VERSIONED** (it commits + syncs to peers — DDR-054). So the download is a **confused-deputy sink**; fetch with rails, never a bare `curl -fsL`:
  > ```bash
  > # HTTPS only · no redirect into internal hosts · bounded size · bounded time · agent-chosen safe filename
  > SAFE="a$(printf '%02d' "$i").${EXT}"          # agent-generated slug — NEVER the URL/anchor/caption verbatim
  > DEST="$DR/assets/moodboard/$DS/$SAFE"
  > case "$DEST" in "$DR/assets/moodboard/$DS/"*) : ;; *) echo "path escape, skip"; continue ;; esac  # in-dir assertion
  > curl --proto '=https' --max-redirs 0 --max-filesize 8000000 --max-time 20 \
  >      -fsS -A "<browser UA>" -o "$DEST" -- "$URL"
  > ```
  > - **HTTPS + no redirects** (`--proto '=https' --max-redirs 0`) blocks `file://` reads and redirect-to-`169.254.169.254` / RFC-1918 SSRF. **Reject a URL whose host resolves to a private / link-local / loopback range** before fetching (`file:`, `http:`, `169.254.*`, `10.*`, `172.16–31.*`, `192.168.*`, `::1`, `localhost` → skip, log to bypass-log).
  > - **Filename is agent-chosen** (`a01.jpg`, `b03.png` — ext validated against the sniffed content type), **NEVER** the URL basename, `anchor`, or `caption` (those carry `../`, NUL, shell metacharacters). The `case` in-dir assertion is the backstop.
  > - **Pass the URL after `--`** and quoted, so a `-`-leading or option-looking URL can't smuggle a curl flag.
  > - **Captions / anchor names are DATA, not code.** A poisoned `caption` / `anchor` / `why_relevant` string is rendered as a JSX **text child** (`{caption}`) — never interpolated into a `src`, a `style`, `dangerouslySetInnerHTML`, an event handler, or any executable position. Treat every research-payload string as inert quoted content (DDR-130 output-handling guard).
  > - A URL that fails the rails is simply **skipped** (the `onError` scrap + provenance backbone keep the slot legible) — a blocked image never blocks the board.
