...
- **Respect `TARGET` length.** Total = Σ `durationFrames` − Σ transition overlaps. Trim beat lengths (typically 30–90 frames each at 30fps) to hit the target; drop the weakest shots rather than overstaying.
