...
- **`React.*` requires an explicit import.** Any use of `React.useState`, `React.useId`, `React.Fragment`, `React.useEffect`, `React.CSSProperties`, etc. MUST be backed by `import React from "react"` (or the named form, `import { useState } from "react"`). A bare `React.*` with no import transpiles clean (TS sees the global type) but throws `ReferenceError: React is not defined` at module-eval in the browser — a runtime crash the structural critic never sees. **Prefer named imports** (`import { useId, useState } from "react"`) and call them unqualified; if you reach for the `React.` namespace, import the default.
