...
3.5. **Stage 4 — design-language moodboard (direction gate).** Before any scaffold, the main agent assembles a **chaotic, hand-assembled collage / pinboard** as a **persistent UI canvas** at `ui/<ds>-moodboard.tsx` (overlapping torn/taped/pinned scraps, scattered paint-chip swatches, ripped type specimens, the signature-treatment hero, voice/feeling-word scraps, marker scribbles + a **dense reference provenance layer** — researched anchors with name + why + source link, not just thumbnails — on a corkboard/paper-grain texture; deliberately messy + human, NOT a tidy poster or card grid), screenshots it, Reads it, and gates on a feeling call: `Jdeme do toho / Uprav / Ukaž víc variant / Tohle ne`. **The moodboard is a normal canvas** — it stays after the bootstrap so the user can **revisit + comment** on it (DDR-080 amendment 2026-07-01); it is never written under `system/<ds>/`. **Default = 1** recommended direction, **main-agent-assembled** (cheap, one `<DCArtboard>`). The user can ask for **2–3 variants** — each generated by its **own blind sub-agent fired in parallel** with a distinct seed pole and self-harvested imagery, so the variants genuinely diverge; the main agent composes them into **2–3 `<DCArtboard>`s side by side in that one canvas** — then pick/mix. ~1–3 min (3–6 for variants) vs the ~30–40 min scaffold — the user approves the *direction* before it's generated; on approval the moodboard is the locked direction contract for Batch A. Interactive-only (skipped on `--no-discovery`; `--quick` auto-proceeds but still asks). **Canonical spec in `plugins/design/skills/design-system/_bootstrap.md` § "Stage 4 — Design-language moodboard" — this is a pointer.**
