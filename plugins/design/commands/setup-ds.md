...
3.5. **Stage 4 — design-language moodboard (direction gate).** Before any scaffold, the main agent assembles a throwaway **chaotic, hand-assembled collage / pinboard** under `_moodboard/` (overlapping torn/taped/pinned scraps, scattered paint-chip swatches, ripped type specimens, the signature-treatment hero, voice/feeling-word scraps, marker scribbles + a **dense reference provenance layer** — researched anchors with name + why + source link, not just thumbnails — on a corkboard/paper-grain texture; deliberately messy + human, NOT a tidy poster or card grid), screenshots it, Reads it, and gates on a feeling call: `Jdeme do toho / Uprav / Ukaž víc variant / Tohle ne`. **Default = 1** recommended direction, **main-agent-assembled** (cheap). The user can ask for **2–3 variants** — each generated by its **own blind sub-agent fired in parallel** with a distinct seed pole and self-harvested imagery, so the variants genuinely diverge — then pick/mix. ~1–3 min (3–6 for variants) vs the ~30–40 min scaffold — the user approves the *direction* before it's generated; on approval the moodboard is the locked direction contract for Batch A. Interactive-only (skipped on `--no-discovery`; `--quick` auto-proceeds but still asks). **Canonical spec in `plugins/design/skills/design-system/_bootstrap.md` § "Stage 4 — Design-language moodboard" — this is a pointer.**
