...
1. **Plugin namespace added by Claude Code itself at registration** — every command's `name:` field is the bare slug (`edit`, not `design:edit`); Claude Code prepends `design:` automatically (per [DDR-191](../../.ai/decisions/DDR-191-revert-plugin-name-prefix-claude-code-now-namespaces.md), which supersedes the older [DDR-006](../../.ai/decisions/DDR-006-plugin-namespace-in-name-frontmatter.md) — that DDR had us bake the prefix in by hand as a workaround for a since-fixed Claude Code bug; doing so today stutters the namespace twice in autocomplete).
...
| `/design:import` | Bring an existing design into Maude. `--reconstruct <image>` (experimental) turns a Figma-frame PNG into a token-styled canvas via a Bash-free vision-authoring + reality-check loop — see [DDR-174](../../.ai/decisions/DDR-174-vision-reconstruction-trust-boundary-and-experimental-posture.md). Token/brand ingestion have their own dedicated entry points (`maude design import-tokens`, `/design:setup-ds --from-brand`), not this command. |
