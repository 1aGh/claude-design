...
### Task 1: RECORD the DDR superseding DDR-104 §2 for Designer-mode scope
...
### Task 2: EXTEND `row()` with an optional label override (additive)
...
### Task 3: ADD `filter` (blur) and `mix-blend-mode` as new CSS rows (available in BOTH modes)
...
### Task 4: BUILD the mode-toggle state + persistence
...
### Task 5: BUILD the Designer-mode render tree — Auto layout, Size, Position clusters
...
### Task 6: BUILD the Designer-mode render tree — Fill, Stroke, Corner radius, Effects, Opacity clusters
...
### Task 7: BUILD the Designer-mode render tree — Text, Spacing, Media clusters
...
### Task 8: DS specimen — extend `components-inspector-controls` with the mode toggle + a Designer cluster example
...
- [ ] Task 1 DDR recorded, cross-references DDR-104, scope precisely stated
- [ ] Tasks 2-3 land with zero Advanced-mode regression (pixel-identical screenshot diff)
- [ ] `filter`(blur)/`mix-blend-mode` round-trip correctly, usable from both modes
- [ ] Mode toggle persists via `maude-cp-mode`, defaults to `advanced`
- [ ] All 11 Designer-mode clusters render correctly per the cluster map, common vs disclosure split matches the Design Decisions table
- [ ] DS specimen documents the mode toggle + at least one Designer cluster
- [ ] `a11y-auditor`: 0 blockers
- [ ] `design-system-guard`: 0 blockers
- [ ] Committed client bundle is release-minified
- [ ] No DDR-worthy decision left unrecorded beyond Task 1's DDR
