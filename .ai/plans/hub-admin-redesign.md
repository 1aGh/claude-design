...
- [ ] All tasks completed
- [ ] `bun test test/admin-size.test.mjs` passes (< 15 KB gz)
- [ ] `bun test test/admin-static.test.mjs` passes (all 5 assertions)
- [ ] `pnpm lint` clean on changed files
- [ ] Hub admin visually matches Sync Hub Admin design artboards:
  - Sign-in: hub-hero lockup, field, btn--primary with ⏎ shortcut hint
  - Bootstrap: hub-hero, fp fingerprint card, Claim hub btn + EXPIRES 24H badge
  - Dashboard: 2×2 tile grid with tile-hd SKU strips, status kv grid, token table with Rotate buttons
  - Token modal: issued-credential dialog with cmd block, Copy button, tear line, raw token stub
- [ ] `dist/admin/` matches `src/admin/` (no stale dist)
- [ ] No regressions in other hub tests (`bun test` full suite passes)
