...
#### Task 2.7 (follow-up): One-click LOCAL-subtitle setup — a "Download & enable" button
> **Owner want (2026-07-11).** Local subtitles must be **one-click**: a button in Settings that downloads the model and it just works — no manual `brew install`, no hand-fetching a ggml file from Hugging Face, no `--model` path wrangling. Today (shipped Phase-2) the local path needs all three by hand — this task closes that onboarding gap. **DDR-worthy** (native-whisper.cpp-vs-WASM engine; which/what-size model; bundle-vs-download; multi-GB consent; storage location).

- **Two mechanisms — recommend B for the true "just works" bar:**
  - **A. Native whisper.cpp + managed model download.** Keep the fast native binary but add a **"Download model"** button that fetches the ggml model (e.g. `ggml-large-v3-turbo.bin`) from a pinned host into a Maude-managed dir (`~/.config/maude/whisper/`, or a runtime `_whisper/` — DDR-115 IGNORED), with consent + a progress bar; `_transcribe.mjs` then auto-resolves it (no `--model` needed). STILL needs the whisper.cpp *binary* → also guide/auto-install it (`maude doctor --fix` per-OS, or bundle a per-platform binary like the agent-browser externalBin pattern). Fast, but two moving parts.
  - **B. Zero-install WASM/JS whisper (RECOMMENDED).** Run whisper in-process via a WASM/JS build (transformers.js Whisper, or whisper.cpp-WASM) so the ONLY setup is a **model download that auto-fires on first use** — NO system binary, NO brew. This mirrors the ML pattern Maude ALREADY ships: `@imgly/background-removal` (DDR-161) lazy-loads client-side and auto-downloads its model. Same shape → "click enable, it downloads once, then transcribes offline forever." Slower than native whisper.cpp, but it genuinely *just works* with one button; offer native (A) as the optional "make it faster" upgrade.
- **UX**: Settings → **Local subtitles** card → **"Download & enable"** → a size-aware consent ("~1.5 GB, one-time, stays on your machine") → progress → done. After that, choosing `--provider whisper` / the Settings "Local" radio (Task 2.6) needs nothing else. The model lives in a Maude-managed, gitignored location (never `assets/`, never committed). A "Remove downloaded model" affordance to reclaim disk.
- **Pattern**: `@imgly/background-removal` lazy-load + model-host fetch (DDR-161, `PhotoLayer`/`photo-bg-remove.sh`); the Phase-5 local-engine multi-GB download-consent + "engine card" UX (§Task 5.2) — reuse that consent component; agent-browser's bundled-`externalBin` per-platform binary shape if going native (A).
- **Gotcha**: the model is multi-GB → HARD consent + resumable/verified download (checksum the ggml). WASM whisper needs the model in a wasm-fetchable form + enough memory; verify on the CSP-split canvas realm if it ever runs canvas-side (it should run MAIN-origin / sidecar, like transcription already does — no canvas surface). Don't bundle the model in npm/the app (bloat) — always download on demand.
- **Validate**: fresh machine, no whisper installed → click "Download & enable" → model downloads with progress → `maude design transcribe --provider whisper` (or the Settings Local radio) works offline with zero further setup.
