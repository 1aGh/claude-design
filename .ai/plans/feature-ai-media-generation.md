...
> **🔬 Task 2.7 SPIKE DONE — PASSED (2026-07-11); BUILD gated on an owner packaging decision.** The "pokud to jde" feasibility spike ran transformers.js `onnx-community/whisper-base_timestamped` (q8) in the Node/Bun sidecar on the JFK clip: **word-level timestamps ✅ accurate** ("And" [0.22–0.52] "so" [0.52–0.84]…), **transcript verbatim**, **2.66× realtime** (4.1 s for 11 s), **~676 MB peak**, **runs main-origin/in-process** (onnxruntime-node) — all four gate criteria met → approach B is feasible. Build notes: word timestamps REQUIRE a `_timestamped` model variant (cross-attention export); `base` is the size default; a cosmetic onnxruntime-node teardown SIGABRT needs a clean `process.exit(0)`. **The one genuinely consequential fork remains open (surfaced to owner):** transformers.js in Node pulls **onnxruntime-node** (heavy per-platform native `.node` binaries) which fights the `bun --compile` standalone-binary distribution (DDR-045) + bloats the npm tarball — vs (a) transformers.js **onnxruntime-web (WASM)** in the browser proof-canvas harness (mirrors the shipped `@imgly/background-removal` precedent, no native dep, slower) or (b) native **whisper.cpp + a managed model-download button** (fallback A, fastest, needs the binary). The engine is NOT built until the owner picks the distribution shape. Spike evidence: `apps/studio/generation/whisper-spike-results.md`.
