...
**T12: GENERALIZE `--imprint` into a file-backed brand prior + in-app upload panel** — 🟡 CLI/markdown layer DONE 2026-07-14, in-app panel NOT started
...
- **Tests**: `apps/studio/test/import-brand.test.ts` (25 cases, pure functions — the exhaustive attribute grammar table incl. the exact shadow/transform-style bypass PoCs from DDR-173's own security review, both now rejected) + `import-brand-browser.test.ts` (2 cases, real agent-browser raster fallback, live-verified end to end against a real wordmark logo: vector loses "Acme" text, PNG carries it as pixels with real PNG magic bytes). 0 new tsc/biome findings against baseline; full `apps/studio` suite 2633 pass (2 pre-existing shared-machine-load flakes, unrelated).
- **Not yet done**: the in-app **Brand-upload panel** (T8's stub in `SetupChecklist.jsx` still links to public docs) — needs a new privileged HTTP route (raw-bytes body, mirroring `/_api/import-asset`'s shape) since `_import-brand.mjs` is currently CLI-only, a Tauri native file-picker command (mirroring `pick_media_file`, memory `reference_tauri_file_open_needs_rust_dialog`), and the actual React panel UI + live verification. DDR-141 brand specimen writing for `preview/logo.tsx` itself stays agent-authored during the normal scaffold flow (per the roster-note above) — this task's script/module scope is asset-extraction only, not specimen-content generation.
...
- **Validate**: `/design:setup-ds test --from-brand <fixtures>` produces a DS whose accent/type/logo trace to the upload; completeness-critic passes. Not yet run live — needs the in-app panel or a manual CLI walkthrough of the full Stage 2 flow first.
