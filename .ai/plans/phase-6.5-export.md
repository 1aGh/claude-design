...
- **Validate:** Export 3-artboard canvas → `.pptx` opens in PowerPoint, Keynote, and Google Slides. Text is editable. Shapes preserve fill/stroke. Reload-roundtrip (open + save in PowerPoint, re-import to mdcc via no-op) doesn't crash anything.
...
- **Validate:** `mdcc design export canva --canva=raster --out ~/handoff.zip` produces the legacy bundle.
