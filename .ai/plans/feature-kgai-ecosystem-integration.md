...
- **Gotcha**: **sync only at `done`/`pause`**, never per-edit (rebuild grows with log; `as-of` is O(n), 12 min @20k — never on a hot path). `status`/`resume` use `kg history`/`context` (flat ~50 ms), never `as-of`.
...
#### Task 9: IMPLEMENT scope tagging + scope-biased search
- **Do**: In `kgai-backend`, every ingest appends `repo:`/`dept:` upserts + `IN_REPO`/`IN_DEPT` links from `config.scope`; every read re-ranks/filters to `scope.dept` first with an `--all-scopes` widen. Interim = Cypher over `IN_DEPT`; file the native `--scope` upstream request.
...
#### Task 15: BUNDLE `kg` + libkuzu as per-platform sidecars
- **Do**: Ship per-platform `kg` binary + native `libkuzu` as `externalBin` sidecars — mirror the compiled-bun / agent-browser model. Add `apps/desktop/scripts/sync-kg.mjs` to the `beforeBuildCommand` chain (next to `sync-sidecar.mjs`) + staging in `stage-resources.mjs`; register in `tauri.conf.json` `externalBin`. libkuzu is a native dylib → explicit binary staging + `@loader_path`/rpath fixup + macOS codesign/notarize + JIT/hardened-runtime entitlements (same class the Bun sidecar hit). `install.sh` (Go/network) is **dead terminal-less** — never call it in-bundle.
...
4. **kgai maturity risk.** kgai is young (memory `project-kgai-research-verification`). Mitigation baked in: opt-in `auto`, `.ai/` fallback + archive always intact, sync-at-close only, `as-of` off hot paths. Re-run `scripts/kgai-smoke/run.sh` after upstream perf fixes before widening rollout.
