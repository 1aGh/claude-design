...
- [x] `/validate` static+test+build gates pass:
  ...
  - [ ] `scenario-runner`: spawned during `/flow:done` but not waited out to completion (user asked to close the plan without further redundant re-verification, given 4/5 fan-out agents had already re-confirmed 0 blockers on a confirmatory pass matching `/flow:execute`'s own thorough live results). Equivalent live coverage already obtained: `design-system-guard` + `a11y-auditor` both drove the real Photo tab (⌘-click selection, typed/drag/keyboard interaction, screen-reader-tree checks) against a live throwaway server (twice — once during `/execute`, once confirmatory during `/done`), and the headless CLI path (`photo-bg-remove.sh`) was manually live-tested end-to-end (all 6 Stage-A hardening behaviors + the real exploit string, pre- and post-fix, by both a human-directed session and `ethical-hacker` independently). Not a formal scenario-runner report artifact — acceptable gap, flagged here rather than silently dropped.
...
- **A re-audit gate genuinely earns its keep.** Both the design-system-guard/a11y-auditor re-run (Task 23) and the security-auditor/ethical-hacker fan-out caught real, non-obvious regressions this session's own work introduced (the `Slider` `onCommit` stale-closure bug, the `$SESSION`-before-trap ordering bug) — neither would have surfaced from tsc/biome/the existing test suite alone. Treat "re-audit after refactor" as load-bearing, not ceremony, especially when a refactor changes a control's interaction model (native `<input type=range>` → custom scrub field).
- **Two independent reviewers finding the identical bug is a strong signal, not a coincidence to shrug off.** security-auditor and ethical-hacker converged on the exact same `$SESSION` line independently — worth trusting that kind of convergence fully rather than treating it as redundant confirmation.
- **Live-browser verification on a shared, concurrently-edited repo has real teeth.** Spawning agent-browser sessions against what looked like an available dev server actually mutated tracked content (a dogfood asset's `.photo.json` sidecar) and regenerated committed dist bundles into dev-mode bloat multiple times over the session. Both were caught and cleaned up, but the lesson: on a shared tree, always re-check `git status`/`git diff --stat` immediately before any commit, even after "everything looked clean" a few tool calls ago — state can drift from processes you don't control.
- **`git commit -- <pathspec>` is the correct tool for committing on a dirty, concurrently-staged tree** — verified empirically (sandbox test) that it excludes already-staged unrelated content rather than sweeping it in. Worth remembering as the default move whenever `git status` shows another session's staged files.
- **`/flow:done`'s whole-repo static/build gates (`pnpm lint`, the site build) are not scoped to "this plan's diff"** — on a fast-moving shared `main`, they will surface (and can only be attributed to, not fixed by, the closing session) unrelated concurrent work's gaps. Confirmed via `git log <pathspec>` before treating any such failure as out-of-scope; a future `/flow:validate` revision could usefully offer a diff-scoped lint/build mode for exactly this shared-tree situation.
- **Re-running the full Step-4 fan-out from `/flow:done` after `/flow:execute` already did an equivalent live pass was mostly confirmatory** (4/5 agents reproduced the same verdicts near-verbatim) — valuable as independent re-verification of the *committed* code specifically, but the user correctly flagged the wall-clock cost of waiting out a 5th (`scenario-runner`) when the confirmatory signal was already overwhelming. A faster `/done` might skip agents whose exact scope was already covered by a very recent (`/execute`-era) pass with matching commit content, reserving the full fan-out for cases where `/execute` didn't already do live verification.
