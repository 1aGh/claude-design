...
- **What worked well:** the plan's own root-cause diagnosis (position-pin-without-restore across 5 shims) was exactly right, and Phase A's per-task Edit-Verify structure caught a second, unplanned bug immediately — `_export-jobs/` sitting under `designRoot` meant `project-raw`'s walk bundled every prior export's bytes into the next one, causing real exponential blowup (28ms → timeout by request #20). Found via a manual timing repro, not a hunch; fixed with one line in `scope.ts`'s `RAW_EXCLUDES`. Worth calling out because nothing in the plan's task list would have caught it — it only surfaced because Task 8's queue design was tested against a realistic multi-request sequence, not single-shot.
- **What worked well:** the live verification against the real `/Users/iagh/git/alligators` repro (Task 26) was the single highest-value step in the whole plan — it's what actually proved the fix (page 1/11/22 visually distinct and correct out of a 22-page PDF) and caught the exponential-blowup bug in the first place. A plan this size with only unit/integration tests and no live-repro step would have shipped with real doubt about whether the reported bug was actually fixed.
- **What worked well:** running the security fan-out (defender + attacker, independent) concurrently with the code-simplifier, all reading the SAME uncommitted diff before any fix landed, meant the attacker's report showed up as independent confirmation of the defender's MEDIUM finding rather than a rehash — worth the ~5 minutes of wall-clock to run genuinely in parallel.
- **What to change next time:** two follow-up UI requests arrived AFTER the initial `/flow:execute` closed out ("toast when export starts", "spinner on the Save button") that, in hindsight, were implicit in the plan's own Solution C section (`"Progress display rule"`, `"Completion finalize rule"`) but weren't explicitly itemized as their own tasks. A plan for a background-job UI feature should explicitly enumerate the full state-transition matrix (queued/running/done/failed × toast/badge/panel/button) as acceptance criteria, not just describe the steady-state end result — would have caught both gaps during `/execute` instead of a follow-up round.
- **What to change next time:** discovered mid-session that this repo's `.ai/state/STATE.md` and `apps/studio/dist/*` are genuinely hot under concurrent sessions — another session landed 2 commits (including its own STATE.md edit) while this one was mid-flight, and a routine dev-server boot (for live UI verification) silently clobbered the just-rebuilt release bundle back to dev-form twice. Neither caused data loss here because `git status`/`git diff --stat` were checked before every stage, but a plan or `/execute` run touching `apps/studio/client/**` should budget an explicit "rebuild `--release` + verify `git diff --stat dist/`" step immediately before the final commit, not just once mid-session — this is the second feature in a row (after DDR-150-era work) where that exact gotcha bit.
- **What to change next time:** the plan's own "Progress display rule" anticipated real per-artboard progress for `canvas-as-separate` but didn't specify HOW a shim reports it back through a spawned-subprocess boundary; the `MAUDE_PROGRESS` stdout-line protocol + incremental-read `spawnShim()` design was invented during `/execute`, not planned. Worth a one-line callout in future plans that touch spawned-subprocess progress: "define the wire protocol between the child process and the parent before writing either side."
