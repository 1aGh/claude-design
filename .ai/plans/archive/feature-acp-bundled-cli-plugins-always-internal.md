...
- `.ai/archive/decisions/DDR-143-acp-session-scoped-plugin-auto-bootstrap.md` — Why: Decision 3 ("no-op for power users") is what this feature supersedes in part; needs a "superseded in part by DDR-\<N\>" pointer per this repo's own convention.
- `.ai/archive/decisions/DDR-166-zero-terminal-acp-cold-start.md` — Why: Decision 1 (append-not-prepend rationale) is what this feature supersedes in part. **Note:** this DDR's T0b code is currently uncommitted, owned by a concurrent session per `.ai/state/STATE.md`'s note about `feature-unified-settings-modal` WIP sharing the tree — coordinate rather than clobber (see Task 2 gotcha).
...
- `.ai/archive/decisions/DDR-167-acp-bundled-cli-and-plugins-always-win.md` — records this policy reversal (exact number TBD at execute time — verify against `.ai/archive/decisions/README.md` immediately before writing, per memory `project_ddr_numbering_races_on_shared_main`; DDR-166 is highest as of this plan's authoring).
...
### Task 1: CREATE `.ai/archive/decisions/DDR-167-acp-bundled-cli-and-plugins-always-win.md` — ✅ completed (as DDR-168; DDR-167 was claimed by a concurrent session)
...
- **Gotcha**: Verify the DDR number is still free (check both `.ai/archive/decisions/` dir AND any uncommitted README index diff) immediately before writing AND again before the closing commit — this tree has concurrent sessions active (memory `project_ddr_numbering_races_on_shared_main`).
- **Validate**: File exists, cross-links resolve, `.ai/archive/decisions/README.md` gets an index entry.
...
### Task 7: UPDATE `.ai/archive/decisions/DDR-143-...md` and `.ai/archive/decisions/DDR-166-...md` — supersession pointers — ✅ completed
...
- **DDR number collision, exactly as the plan's own gotcha warned.** DDR-167 was claimed by a concurrent session before this plan's Task 1 ran — caught immediately by re-checking `.ai/archive/decisions/` fresh at write time (per memory `project_ddr_numbering_races_on_shared_main`) rather than trusting the plan's own placeholder number. Renumbered to DDR-168 with zero rework. Confirms the memory's own advice is still the right call on a busy shared `main`; nothing to change here.
- **The plan's Validation §5 ("verify against the actual bundled `.app`") was the single highest-value step in the whole plan, and it was initially deferred as "too expensive to run inline."** The user pushed back once ("how do we actually know this works?") and a second time (explicitly chose the live `.app` build over a lighter check) — both times correctly. The live verification found real, non-obvious things static review missed: (a) the `enabledPlugins` guard is empirically a no-op — Claude Code's own `--plugin-dir` precedence is what actually prevents double-registration, not our code; (b) `stage_bundled_cli_link()` never engages under `tauri dev`, so a lighter dev-mode check would have proven nothing for the PATH-precedence half. **Lesson for future plans touching this exact surface (ACP session bootstrap, plugin/CLI precedence):** don't let "the build is slow" default to skipping the native dogfood — budget for it up front rather than treating it as a stretch goal, since this class of change has repeatedly (DDR-166, DDR-045) hidden its real bugs specifically in the compiled-binary-vs-dev-tree gap.
- **A genuinely cheap, reusable verification technique emerged and is worth reusing, not just this once:** sandboxing `~/.claude` state via `CLAUDE_CONFIG_DIR` + sandboxing login-shell PATH resolution via a fake `SHELL` script, both real env vars the app already honors (no new debug backdoors). This let the plugin-precedence claim get tested against the REAL installed `claude` CLI without touching the developer's actual `~/.claude` or global npm state, and without needing a full Tauri build at all. Should be the default first move for any future "does X actually work against Claude Code's real plugin/settings resolution" question, before reaching for a full desktop-e2e scenario.
- **The formal `/flow:done` code-review round (run against the already-committed diff, independent of the 2 rounds already run during `/execute`) was not redundant — it caught two real things the execute-time rounds missed:** a UI copy overclaim (`readiness.ts` said "always active" for a case the code doesn't actually guarantee) and the absence of any CI-level regression coverage for the plugin-dedup behavior the whole DDR rests on. Both are now fixed/documented. **Lesson:** don't treat "already reviewed during execute" as a reason to skip or rubber-stamp `/done`'s own review round, even when the diff hasn't changed — a fresh adversarial pass on a settled diff still finds things a pass that's mid-fix-and-defend doesn't.
- **Shared-tree commit hygiene worked, but was manual and slow.** `README.md`/`STATE.md` had concurrent, unrelated in-flight edits from other sessions at commit time; isolating just this feature's hunks required hand-building `git apply --cached` patches rather than a plain `git add`. It worked cleanly both times, but it's fragile/manual. **Possible follow-up (not urgent):** a small helper script (`scripts/stage-my-hunk.sh <file> <old> <new>` or similar) would make this repeatable instead of ad-hoc Python string-replace + diff + apply --cached each time a shared append-only file (STATE.md, decisions/README.md) needs a surgical commit on a busy tree.
