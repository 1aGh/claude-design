...
- **DDR number collision, exactly as the plan's own gotcha warned.** DDR-167 was claimed by a concurrent session before this plan's Task 1 ran — caught immediately by re-checking `.ai/decisions/` fresh at write time (per memory `project_ddr_numbering_races_on_shared_main`) rather than trusting the plan's own placeholder number. Renumbered to DDR-168 with zero rework. Confirms the memory's own advice is still the right call on a busy shared `main`; nothing to change here.
