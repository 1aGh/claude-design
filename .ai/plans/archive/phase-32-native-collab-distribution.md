...
- **Plan staleness:** the plan's `plugins/design/dev-server/` paths (→ `apps/studio/`, DDR-095) and the `maude.1agh.dev` host (real: `maude.sh`) were both wrong — same DDR-095 drift that bit phases 27–31. A plan-author pre-flight that greps the plan's paths/hosts against the tree would catch this once.
