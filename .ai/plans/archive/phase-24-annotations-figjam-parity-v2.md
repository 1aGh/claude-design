...
- **Single-source geometry paid off.** Putting arrow shaft+head geometry in one `canvas-arrowheads.ts` (`arrowPrimitives()` → `SvgPrimitive[]`, consumed by BOTH the serializer and `StrokeNode`) made the on-disk/on-canvas drift class structurally impossible. Reuse this "one source, two projections" shape for any future dual-representation vocabulary (the polygon stroke followed the same discipline).
- **`bun test` is the authoritative parse gate, not the `tsc` error count.** Two traps cost time: (1) a top-level `.ts` file in a re-export *cycle* with a react `.tsx` silently broke the global `JSX` namespace project-wide under `types:["bun-types"]`; (2) a stray backtick in a CSS template literal that `tsc` tolerated (count stayed at the 3 baseline) but `bun`'s parser rejected — surfacing as a test "hang". Keep `canvas-arrowheads.ts`'s dependency one-way, and treat a green tsc count as necessary-not-sufficient.
- **Chrome-via-canvas-transpile cache invalidation is a recurring footgun.** Cursor/chrome code ships through the on-demand canvas transpile, whose ETag was derived only from canvas source — so a chrome edit served a stale 304 even after a restart ("my edit didn't show up"). Fixed with `RUNTIME_BOOT_ID` + `CHROME_EPOCH` in the etag + a `.ts`-aware watcher. **Plan-template ask going forward:** "does this touch chrome served via the canvas transpile?" → if yes, the etag/watcher question is up-front, not a post-hoc debug.
- **A new untrusted-origin → shell write sink must re-trigger the cross-origin re-audit.** The app-wide cursor was added without re-checking the cross-origin seam inventory; `/flow:validate`'s (cached) security pass predated the cursor code, so the medium clickjacking-aid only got caught by `/flow:done`'s fresh attacker pass. The ethical-hacker's own systemic finding: any new `dgn:*` handler in `app.jsx` is a write into the un-CSP'd trusted origin and should re-run the seam audit. Worth a checklist line in any `/design` plan touching `app.jsx` postMessage handlers.
- **Token-resolution beats string-validation for untrusted cross-origin values.** The clickjacking fix replaced "validate an untrusted cursor string's shape" with "echo a token, resolve it server-of-trust-side from a frozen allowlist." This removes attacker-controlled bytes from the boundary entirely rather than trying to validate them — the reusable pattern for the DDR-054 untrusted-canvas model.
