...
**Wave 1 — correctness + quick wins (DONE 2026-06-12, commit 8c17097 — live-verified in agent-browser):**
- [x] **W1.1 (#1) Selection survives an edit.** Committing a CSS/attr value triggers the file-watcher HMR reload, which remounts the canvas and drops the in-canvas selection halo (panel stays connected, element no longer selected). Add: (a) **optimistic inline apply** — on commit, post the new prop to the iframe so the element updates instantly; (b) **reselect-after-reload** — on the iframe's `dgn:'loaded'`, re-post `select-by-id` with the live `selected.{id,artboardId,index}` to restore the halo.
- [x] **W1.2 (#11) Custom CSS props + HTML attrs are written but invisible in the panel.** `styleMapsFor` only reads the 14 `KNOB_PROPS` → an authored prop outside that set (or a custom HTML attr) never round-trips into a row. Read ALL inline style props + collect non-style attributes (`data-*`/`aria-*`/`role`/`title`/…) into the selection; render authored-but-uncurated props + custom attrs as their own rows.
- [x] **W1.3 (#4) Stale Inspect disclaimer.** `InspectorPanel` Inspect tab still shows "Computed fill / radius / type readout lands with the live CSS bridge (Phase 12)" — Phase 12 shipped. Replace the callout with the real computed fill / radius / type readout (mirror `Studio.tsx` `[data-dc-screen="handoff"]` Inspect mock).
- [x] **W1.4 (#2) Reset to original.** Per-row reset (× / revert) that DELETES the inline prop (back to class/inherited) + per-section reset (e.g. Layout). Needs a new delete path in `editStyleProp` (remove the property node) + `editStringAttr` (remove the attribute) wired through `/_api/edit-css` + `/_api/edit-attr` with a `reset` flag. Add canvas-edit tests.
- [x] **W1.5 (#10) Remove the "link all sides" button** + the `link` state (Webflow replaces it with alt-drag, W2). Default to per-side editing.
- [x] **W1.6 (#9a) Spacing shows 0 / resolved values** (Webflow-style filled box, screenshot #13) instead of blank when a side is unset.
...
## Phase 12.3 — round 2/3 live-iteration + maude-DS alignment (DONE 2026-06-12)

After the Wave-1/2/3 batch the user kept live-testing on `:4555` and drove three more rounds of feedback, each landing as its own commit. All live-verified in agent-browser **and** user-confirmed:

- **maude-DS alignment of the whole right panel** — Inspect/Layers/CSS chrome ported to the maude specimens: coordinate-field cells, prefix-glyph inputs (Figma `[X][Y][⌜corner⌝]` pattern, per `components-inputs.tsx`), hairline material, accent selection bar (`581a5c2`, `c178a37`).
- **One unified Figma-style colour picker** — collapsed the two redundant colour popovers into a single HSV sat/value square + hue slider + eyedropper + hex field, with Custom / Variables tabs; border-colour adopts it too (`606e9f8` → `20b5b40` → `de360ab`).
- **Row-chrome consolidation** — reset + save/error status folded into one interactive **leading provenance dot** (double-click resets, tooltip, success glow); custom-property rows collapsed to a single line with a trailing `×` (`64dabf6`); provenance is outline/glow only, no fill background (`4cdbd46`).
...
- **Canvas-first-then-port paid off twice.** Designing the panel in a `Studio.tsx` artboard before touching `app.jsx` caught composition problems (the two-popover redundancy, the provenance-dot overload) at screenshot speed instead of in the 310 KB bundle. The round-2 maude-DS alignment reused that same loop. Keep "design the chrome as a canvas" as the default for any non-trivial shell-UI work.
- **`whatever is committed ships` is the load-bearing dev-server gotcha.** The DEV-bundle self-heal silently masked real bugs (`6130ebb` empty-key children only crashed under the release bundle the user actually ran). Every wave had to end with `MAUDE_SKIP_RUNTIME_BUILD=1 bun run build.ts --release` + verify the 310 KB artifact. Verification boots MUST use `MAUDE_NO_AUTOBUILD=1` or they overwrite the release bundle — this bit the session repeatedly until it became reflex. Worth a CLAUDE.md line (already present) and a memory.
- **The trust boundary is wider than the write routes.** The `/flow:validate` pass hardened the edit-* HTTP surface (F1–F3); it took the `/flow:done` adversarial pass to notice the NEW W1.1 optimistic-apply postMessage path reopened the same DDR-054 boundary from the client side (A1 window-global desync). Lesson: every new shell↔canvas message contract is a trust-boundary surface — audit the message vocabulary, not just the HTTP routes. Captured as review-log follow-up 4.
- **Colours must be detected by VALUE, not name.** The Variables list missed tokens whose names didn't contain "color"/"bg"; switching `isColor` to sniff the resolved value (`#hex`/`rgb()`/`oklch()`/keyword) lifted 47→51 tokens. Token-name heuristics are a recurring trap (see the `css-var-alias-scope-trap` memory).
- **Long live-iteration sessions need the bundle-restore dance written down.** Multiple times a test boot churned `dist/` and the recovery (`git checkout -- apps/studio/dist/` then rebuild release) cost minutes. The `MAUDE_NO_AUTOBUILD=1` boot recipe should be the documented default for any in-session verification, not rediscovered each time.

### Open follow-ups (not blockers — user has not directed these)
1. Two critique "bigger-bets" left undirected: icon-segmented align/justify/flex-direction controls; Inspect-tab "computed answer-key" view. Surface on next `/design:edit` if the user wants them.
2. Security review-log items 1–4 (F1 relay confused-deputy; residual-F2 on other write routes; F4 test-harness port collision; message-vocabulary source-gate audit) — all cross-cutting, own plans.
