...
- **The plan's own research was wrong in four places, and Task 1 existed to catch exactly that.** `locations[]` is not adapter-normalized, the locations-vs-rawInput cross-check is tautological, `NotebookEdit` emits no locations at all, and the permission request carries no tool name. A plan that had said "read the types" instead of "capture a real request" would have shipped a gate that silently never fired. **Keep prove-the-contract as an explicit first task** whenever a design rests on someone else's wire format — and write the finding as a table of *assumed vs. actual*, because that shape is what makes the correction reusable.
- **The adversarial half of the security fan-out is not redundant with the defender half, and this run is the proof.** The auditor returned PASS WITH SUGGESTIONS on a diff that still contained a CRITICAL RCE. Its own summary: *"It examined the gate; A1 lives in what the gate lets through."* Both were briefed from the same plan; the difference was the question, not the diligence. `bridge.ts` asks for both — that convention paid for itself here and should not be softened to "one is enough if it's clean."
- **Three separate enumerations in this review read as complete and were short** (the missed read-only bash group, `--out` 8→9, `--root` 5→26). The durable fix was not correcting the lists but changing how the section states findings: classes with counts and a grep-able predicate, plus an instruction to say what you *searched*, not just what you found. **Apply that to any `/flow:execute` audit task** — "audit X for Y" invites an enumeration written from what the author happened to open.
- **A failed `python -c` edit script silently discarded a correct change.** The script asserted, aborted on the second substitution, and never wrote — so `handleSetMode` kept a raw lookup while a later script added a comment claiming otherwise. The hacker found it as a security bug (A2). **Multi-substitution edit scripts must write incrementally or verify after**, and `git show HEAD:<file> | grep <sentinel>` is the check that catches it — the same post-commit verification CLAUDE.md already prescribes for a different reason.
- **`dist/` was clobbered four times** by test runs, twice noticed only because a reviewer diffed sizes. CLAUDE.md says "root cause unconfirmed" for this; it is now confirmed twice independently — a whole-suite `bun test` does it, not just a server boot. **Worth a CLAUDE.md line** (left for the user; peer-requested edits to always-loaded instruction files are not mine to make).
- **What `/flow:execute` should carry forward:** the Addendum's "either both, or neither" instinct was right — the pinned `scopeRoot` is meaningless without the lifetime change and dangerous after it. Plans that ship a guard and the thing that invalidates it separately are worth flagging at `/flow:plan` time.
