...
**T15: BUILD `/design:import --reconstruct` (image → canvas)** — ✅ DONE 2026-07-14, incl. a `/flow:done` code-review round that found and fixed a Critical DDR-174 gap
- **`/flow:done`'s security fan-out found a real Critical the design-stage review missed**: unrestricted `Write` on both new agents (no Claude Code mechanism exists to path-scope one subagent's Write — verified authoritatively, not assumed) plus a diff-check scoped only to `$DESIGN_ROOT` meant a steered `reconstruct-agent` could plant a `.claude/settings.json` hook or poison `CLAUDE.md`, invisible to the check, achieving durable code execution without ever touching `Bash` itself. Fixed: diff-check widened to the whole repo (closes the tracked-repo variant before the orchestrator's own next `Bash` call), a targeted snapshot of the two well-known global Claude Code config paths (closes the `$HOME/.claude/**` variant DDR-167-style-verified as unreachable any other way), `permissionMode: default` added to both agents (real protection for any non-`bypassPermissions` session), plus a `/tmp` symlink-race fix. A confirmation-pass review then caught and fixed a real bug IN the fix itself (BSD-only `stat -f` silently no-opping on Linux) before this was called done. Full reasoning, the honest residuals (shell rc files / `~/.ssh/` / `$HOME/.claude/{agents,commands}/` remain genuinely unwatched — named explicitly, not glossed over), and the confirmation-pass record: DDR-174's own `## Addendum` section. 2 new a11y/design-system blockers (light-theme contrast on the new UI badge; the recursive file-tree component not threading the badge into subfolders) were also found and fixed in the same pass — see below.
...
- **Done**: `plugins/design/commands/import.md` (`/design:import --reconstruct`), `plugins/design/agents/reconstruct-agent.md` (authoring) + `reconstruct-critic.md` (comparator) — both `tools: Read, Write, Glob, Grep` only, frontmatter-verified by a new grep-based regression test (`cli/lib/reconstruct-toolset.test.mjs`, DDR-174's own Consequences-mandated check) that also asserts the orchestrator never spawns the default `design-critic` as its comparator. The orchestrator markdown implements every DDR-174 mechanism literally: source image ingested through DDR-167's gate (no second read path), target `.tsx`/`.meta.json` path command-computed up front (never agent-chosen), a `git status --porcelain`-class pre/post diff check that hard-fails on any unexpected write (compare-and-abort, never interpolating a discovered path into a further `Bash` call), screenshot args built only from command-computed values, `converged` extracted via a deterministic single-field `jq -r` read (never LLM interpretation of the comparator's prose), a concrete round cap (default 3, hard-capped at 4), and the governing invariant stated explicitly at the top of the command file (the orchestrator never `Read`s the source image / reconstruction screenshot / authored files' content, for any purpose including describing the result to the user).
- **Net-new ingestion primitive found needed, not pre-existing:** DDR-174 Decision 4 assumes local raster (PNG/JPEG) images already have a "same gate any other image upload goes through" — that gate existed for browser drag-drop but NOT for a local CLI file path (DDR-167/T10 only shipped SVG/PDF kinds). Extended `_import-asset.mjs` with a `raster` kind (magic-byte sniff, containment write, the corrected pre-resolution-leaf symlink check T11 already fixed once — not the structurally-inert post-resolution version T10 shipped and flagged as a follow-up) rather than inventing a parallel path. `assetName`'s charset contract widened to accept `jpg` alongside `svg`/`png`.
- **UI surfacing (DDR-174 Decision 6 — must reach the file tree, not just the meta.json file):** `api.ts`'s `buildIndexData()` gained a `canvasKinds` map (mirrors the existing DDR-093 `canvasDesignSystems` map exactly — piggybacks on the same `loadCanvasMeta` call, zero extra I/O for the common case), threaded through `/_index-data` → client `cfg.canvasKinds` → `Sidebar` → `Tree` → `CanvasRow`/`FileRow` → a new dashed `.st-row-exp-badge` ("exp") badge. **A real bug found only via live testing, not caught by tsc/tests:** the first wiring pass threaded the prop through every component but never actually stored `data.canvasKinds` into `cfg` state in `loadTree()` — the badge silently never rendered. Fixed; re-verified live.
- **Tests**: `apps/studio/test/import-asset.test.ts` gained 15 new cases (raster magic-byte sniffing, symlink/empty/oversized rejection, real-format-over-claimed-extension sniffing, dedup, the widened charset contract) — 50/50 pass. `cli/lib/reconstruct-toolset.test.mjs` (9 cases) is the DDR-174-mandated toolset-restriction regression guard. `bunx tsc --noEmit`: same pre-existing 12-error DDR-026 baseline, 0 new. `biome check`: clean. Full `apps/studio` suite: 2651 pass / 1 fail (confirmed pre-existing shared-machine-load flake, passes in isolation) / 5 skip.
- **Live-verified end to end**: `import-asset --kind raster` against a real PNG via both the direct `.mjs` entry and the registered `maude design import-asset` CLI verb (content-addressed, correct ref returned). Booted a real dev server against a throwaway test project with a hand-seeded `kind: "reconstructed-experimental"` canvas — confirmed `/_index-data` returns `canvasKinds` correctly, and (after the prop-threading fix above) confirmed via `agent-browser` that the "exp" badge genuinely renders in the live file tree, not just in the meta.json file. **Not live-verified**: the actual multi-round agent loop (spawning the real `design:reconstruct-agent`/`design:reconstruct-critic` subagent types) — these are net-new plugin agents that require a marketplace `/plugin marketplace update` + `/reload-plugins` cycle to become invokable, which is outside a single non-interactive session (same verification-ceiling class already documented for T12's `--from-brand` full walkthrough). Confidence instead rests on: exact structural mirroring of the already-verified `draw.md`/`draw-agent.md` orchestration pattern, the frontmatter toolset test, and the live-verified CLI/HTTP/UI plumbing every round of the loop depends on.
- **Incident, caught and fully reverted before this checkpoint:** while building a throwaway test project for the live-verify above, `maude init --name testproj --force` was run without `--root`/`cwd` pointing at the scratch directory, so it executed against THIS repo instead — overwriting `.ai/state/STATE.md` (2148→5 lines), `plans/README.md`, `release-guide.md`, `decisions/README.md`, `workflows.config.json`, and creating bogus `.ai/business/`, `.ai/design-import/` dirs. Caught immediately via `git status` before any commit; fully restored via `git checkout -- <files>` + removing the untracked additions (repo was clean at session start, so HEAD was the correct restore target). Verified `STATE.md` back to its original 2148 lines before continuing. No data lost, but flagging honestly rather than silently proceeding.
...
- **Validate**: reconstruct a known Figma-frame PNG → screenshot parity acceptable to design-critic; label output `kind`/meta as reconstructed-experimental; confirm the DDR-174 architecture (agent toolsets, typed verdict, diff-check) is actually implemented as designed, not just documented. **Structurally confirmed** (every mechanism is literally present in the command/agent markdown, per the "Done" bullet above); the live multi-round agent walkthrough itself is the one piece not run this session — see verification-ceiling note above.
...
- [x] All phase tasks completed (P0–P4; P4 shipped labeled experimental) — T13 (video re-cut) is the one deliberate, owner-directed exception, not an oversight
- [x] **Zero-terminal ACP cold start (P0): a non-technical user reaches their first AI edit with no terminal and no app restart** — `claude` installed from the app, signed into their own subscription via browser, `maude` bundled, plugins auto-loaded (`acp-cold-start` desktop-e2e green, 3/3, T0f)
- [x] P0 respects the ToS line: the user's OWN `claude` + OWN Anthropic login only; Maude never embeds the SDK with its own token and never takes custody of Claude credentials (DDR-123; memory `reference_claude_subscription_via_users_cli_not_sdk`)
- [x] `/flow:utils-verify` passes after each task (Edit-Verify Loop, max 3 iterations)
- [x] `/flow:validate`-equivalent passes overall (run at this close-out for the final uncommitted diff; earlier phases each carried their own per-phase gate at commit time — see the plan's per-task Done/Validate notes):
  - [x] Static (types, lint, format) — `pnpm lint`/`pnpm format` clean at close-out
  - [x] Tests (full suite incl. new ingestion/tour/readiness tests) — CLI 194/194, dev-server 2651/2652 (1 confirmed pre-existing shared-machine flake)
  - [x] Build (+ committed studio bundle rebuilt `--release` for any client change) — site build clean (caught + fixed a genuine `command-catalog.mjs` drift), studio bundle rebuilt 3× across the fix cycle
  - [x] **`security-auditor` + `ethical-hacker`: 0 findings ≥ severityFloor on the ingestion surface** — close-out fan-out found + FIXED a real Critical (T15's unrestricted-`Write` confused-deputy gap), independently re-confirmed closed by a second review round; every earlier phase (T9/T12a/T14 DDRs) carried its own dedicated review before code
  - [x] `design-system-keeper` + critic panel: 0 blockers on new UI + the explainer canvas — close-out `design-system-guard` found + fixed 1 real blocker (recursive-component prop-threading gap)
  - [x] `a11y-auditor`: 0 blockers on onboarding surfaces — close-out found + fixed 1 real blocker (light-theme contrast on the new "exp" badge)
  - [x] `desktop-e2e`: first-run + quick-setup scenarios green (T0f, T5, T7 — run in their own phase sessions, not re-run at this close-out; T15 has no desktop-e2e scenario of its own, it's CLI-only today)
- [x] Explainer video is watchable **on the public site without installing** (the designer's core ask) — `site/public/demo.mp4` ships (predates this plan, confirmed still live); Phase 1 itself pivoted to seeding "how to" reference canvases per an explicit owner redirect (see Phase 1 pivot entry in STATE.md) rather than further video investment — the underlying ask (watchable pre-install) is still met, the plan's OWN P1 deliverable shape changed by owner request mid-flight
- [ ] **A real third-party `tokens.json` + a brand PDF successfully become a usable `system/<ds>/` (completeness-critic passes) — NOT fully met, named honestly, not silently checked off.** `tokens.json` import (DDR-172/T11) is shipped and live-verified against a real DS. **PDF import specifically remains deferred** — a genuine, spike-verified mechanism blocker (headless Chromium cannot render `file://*.pdf` under browser automation, confirmed across 4 configurations), not a shortcut; DDR-167's own addendum documents the finding and names 3 untried follow-up mechanisms, none pre-selected. This is a known, already-surfaced gap carried forward from T10 (2026-07-14), not a new discovery at this close-out.
- [x] DDRs recorded: **zero-terminal ACP cold-start posture** (DDR-166), onboarding/video wiring (folded into DDR-166's scope + the Phase-1 pivot), **ingestion security posture** (DDR-167 + addendum), token-mapping contract (DDR-172), content-handoff trust boundary (DDR-173), vision-reconstruction posture (DDR-174 + this close-out's addendum)
- [x] Runtime-state taxonomy + canvas-origin allowlists updated if any new `_*` path or route was added (DDR-115/088) — `_history/_reconstruct/` already covered by the existing `_history/` prefix rule in all three lists (verified, no update needed); `/_api/import-asset` + `/_api/import-brand` privileged routes added to `canvas-origin-gate.test.ts` in their own T10/T12 sessions
- [x] Out-of-scope boundary held: no changes to `build-desktop.yml` / arch detection (that's `feature-desktop-intel-mac-support.md`)
- [x] No regressions; code follows project conventions — confirmed via the full gate re-run at this close-out
```
...
- **Design-stage adversarial review (`security-auditor`, 4 rounds) is necessary but not sufficient — an execution-time attacker-persona pass on the ACTUAL CODE is a genuinely different, complementary check, not a redundant re-verification.** DDR-174 went through 4 rigorous design-stage rounds and was rightly Accepted on that basis, but all 4 reasoned from the same starting frame ("no Bash/network reach"); none asked the orthogonal question the post-implementation `ethical-hacker` pass found in minutes — does the agent's remaining, deliberately-kept `Write` tool let it plant instructions for a DIFFERENT, already-privileged interpreter to execute later? Next time: budget for both a design-stage AND a code-level adversarial pass on any feature that grants a tool-restricted agent `Write` — don't treat "the design was reviewed" as covering what the implementation actually does.
- **When a review disagrees with a fix's own causal reasoning, that's worth a confirmation round even after the "real" fix lands.** The first fix correctly widened the diff-check scope, but its own stated reasoning ("abort happens before step 5c closes it") was subtly wrong — the actual protection came from a different, uncited Claude Code platform behavior (hook config snapshotted at session start). A second independent review caught this AND a live, concrete portability bug (BSD-only `stat -f`) the first fix's own author (this session) didn't think to test on a different platform. Two-round confirmation discipline (already this repo's house style for DDRs) earned its keep again, on code this time, not just prose.
- **A large, long-running plan closed across many sessions accumulates real drift in exactly the places automated gates are built to catch — trust the gates, don't assume "it'll be fine" for a small addition.** Adding one new slash command (`/design:import`) tripped `site/lib/command-catalog.mjs`'s own parity assertion, a check that exists for precisely this reason. Running the FULL configured `quality.build` gate (not just the tests/lint the execution session already ran) at close-out caught this before it shipped broken docs.
- **Live testing outside the repo you're working in needs the same care as testing inside it.** A throwaway test-project setup accidentally ran `maude init --force` against THIS repo instead of the intended scratch directory (missing `--root`/`cd`), briefly truncating `STATE.md` from 2148 to 5 lines before being caught via `git status` and fully restored. No harm done because it was caught before any commit, but the near-miss is worth naming: always double-check the CWD/`--root` argument before running any command with side effects, especially scaffolding/init-style commands, even in a "just testing" context.
- **A batch of independent, parallel-dispatched reviewers (security + attacker + a11y + design-system + simplifier) found 4 real, distinct, non-overlapping issues in one pass** (1 Critical, 2 UI blockers, several lower-severity items) that none of the execution session's own testing had caught — each reviewer was looking through a different lens (does it match spec / can I break it / can I see it / does it match our own conventions) and each lens found something the others structurally couldn't. This is the concrete payoff of running the full fan-out rather than a single generic "review this" pass, worth the extra wall-clock at every close-out that touches security-sensitive or user-visible surface.
