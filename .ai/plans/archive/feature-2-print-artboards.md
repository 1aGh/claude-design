...
- [x] `/flow:validate` static gates — `bun test` in `apps/studio` (2925/2925 pass, incl. the 100 print-scoped tests), `node --test cli/**/*.test.mjs` (195/195 pass), `biome check` clean for every file this feature touched (one pre-existing unrelated warning in `canvas-edit.ts` from 2026-07-05, not this feature). Cross-platform scenario-runner / a11y-auditor / design-system-guard fan-out **NOT run this session** — this feature's UI surface was instead live-verified end-to-end via agent-browser across 4 dogfood rounds (kind switching, guides overlay, PDF export, DPI, shared inspector controls) directly in the user's running session, which is stronger evidence than a synthetic scenario for this feature. Recommend a full `/flow:validate` scenario pass before the next release cut.
- [x] Second dogfood round (Kind-desync real root cause + zoom-read engine split + shared inspector controls) — fixed, live-verified, recorded in DDR-182 Addendum 2
- [x] `/flow:done` code-review fan-out (security-auditor + ethical-hacker + simplifier, parallel) — 1 CRITICAL (arbitrary file write via unsanitized artboard id, PDF-new + PNG-pre-existing sibling) + 1 HIGH (PDF export missing PNG's output-size guard) + 1 MEDIUM (bleedMm/marginsMm unbounded at read time) + 1 LOW (new readFileSync missing path containment) — all 4 fixed before commit + regression-tested (`bin/_pw-launch.test.mjs`, 8 new cases); 1 duplication + 1 stale comment from the simplifier also applied. Recorded in DDR-182 Addendum 3. Full suite re-verified green post-fix (2933/2933).

## Retro

- **What worked:** the single-source `print/units.ts` design decision (T1's "two mm→px sites would desync overlay from export" gotcha) paid off directly during the security fix — `MAX_PRINT_MM`/`clampMm` only had to be added in ONE place (`resolveBleedMm`/`resolveMarginsMm`) to close the read-time bound gap for every consumer (overlay, PDF post-pass), instead of chasing N call sites.
- **What worked:** live dogfooding in the user's own running session found real bugs (Kind-desync root cause, Chromium/WebKit zoom-read split, missing token-binding parity) that a synthetic scenario likely would have missed or under-specified — but it took TWO rounds to find the actual root cause of the Kind-desync symptom (the first "fix" was real but incomplete). Next time: when a dogfood fix doesn't fully resolve a reported symptom, treat the persistence itself as a signal to go one layer deeper (attrs/DOM/echo-merge) rather than assuming the fix just needs polish.
- **What didn't work / risk to flag:** `apps/studio/dist/` got clobbered by an unminified dev rebuild THREE separate times this session — twice from a stray leftover dev-server process, once apparently from `bun test` itself (no server process was found running at the time). The existing CLAUDE.md guidance only warns about booting the source dev-server directly; running the test suite turned out to be a trigger too. Recommend checking `git status apps/studio/dist/` before AND after every `bun test` run in this repo, not just server boots — flagged for a CLAUDE.md update at `/flow:done`'s debrief step.
- **What worked:** the `/flow:done` parallel security review (defender + attacker) earned its keep — the attacker pass found a genuinely CRITICAL, cross-cutting bug (unsanitized artboard-id-as-filename, present in BOTH the new PDF shim and a pre-existing PNG sibling) that the defender pass and all four live dogfood rounds missed, because it required tracing "where does this convenience filename actually come from" across three files rather than pattern-matching a known vulnerability class.
- **What to change next time:** the plan's own T5 comment ("page SIZE never multiplies by [deviceScaleFactor], so the size guard doesn't apply") was a reasonable-sounding but wrong inference baked into a code comment at write time, and nothing caught it until the adversarial review. When a size/resource guard is added to ONE of two structurally similar exporters (PNG got it, PDF didn't in the same task), that asymmetry itself should be a self-review trigger before `/done`, not just a reviewer catch.
