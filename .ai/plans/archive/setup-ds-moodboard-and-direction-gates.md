...
- **The 7 planned tasks landed cleanly on the first pass** (all markdown-spec edits + DDR-079 + a read-only render-path check). Right-sizing held: reachability 2/2, single-canonical-source, no heavy gates (no app/UI/code surface). The plan's own "heavy gates mostly don't apply" Validation note was correct and saved a pointless 5-platform scenario run.
- **Live dogfood was the real value — and it took THREE rounds, not one.** The static spec passed, but rendering an actual moodboard surfaced what prose couldn't: v1 (one main-agent canvas) looked like a tidy app-mockup; v2 (provenance + 3 variants, still one author) was better but homogeneous; v3 (per-variant **parallel blind sub-agents** + **chaotic collage** + each gathering its OWN imagery, 9–12 real images) finally hit "jo, takhle jsem si to představoval". **Learning for `/plan`:** when the artifact is a *taste/feeling* object (a moodboard), spec its messy, divergent shape UP FRONT — the original "single artboard, assembled, no fan-out, <3 min, tidy" constraint optimized for cheapness and produced a machine-made feel. The fix (chaotic collage + per-variant fan-out + image density) was folded back into the canonical spec mid-flight (via a redesign Workflow), so the spec now matches the validated reality.
- **Process insight worth keeping:** one sub-agent writing all N variants → they share a "house style" and feel machine-made; N independent **parallel, mutually-blind** sub-agents (distinct seed poles, pairwise-distinctness asserted) → genuine divergence. This is now in the spec's variant mode.
- **Two of the plan's own deferred bugs were hit first-hand during dogfood** (`server-up` crashing on missing `yjs`; `/_api/export` returning empty without Playwright browsers) — concrete evidence those deferrals are real and should be the immediate Round-2 follow-up.
- **Shared-`main` near-miss:** the first edits accidentally landed in the parent repo on `main` (held the user's concurrent Phase 13 WIP) instead of this worktree. An inode mismatch caught it; verified parent-HEAD == worktree-current before surgically restoring. **Learning for `/execute`:** in a worktree, ALWAYS use worktree-absolute paths for Read/Edit — the plan's relative `plugins/…` paths resolve against the wrong tree otherwise.
