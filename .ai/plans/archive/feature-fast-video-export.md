...
- **The plan file lagged reality by three commits.** Most of this plan (Tasks A1–A4, 1, 2, 4, 6a–d, 8) had already shipped in `3becccd9`/`354ec5c4`/`6b1b7a63` before this close-out session started, but nothing had gone back to check off the plan's own task list or update kg with per-task granularity — `kg context` only had "plan: active" and two debate/decision nodes, no task-level state. The gap was only found by reading the actual source files and `git log -- <touched files>` before trusting the plan doc. Worth carrying forward: **before executing a plan in this multi-session Syncthing repo, diff its task list against recent commits touching the files it names** — a plan can go stale from work landing outside `/flow:execute`'s own checkpoint loop.
- **The validate fan-out earned its keep on a "nothing to check" diff.** The actual code diff (a JPEG knob) touched no UI, so scenario/a11y/design-system were expected to report clean-and-inapplicable — and one of them didn't: the a11y-auditor's informational spot-check of the *already-shipped* `DegradedNote` UI (read only because the whats-new entry pointed at it) caught a real, live bug (`--u-status-warning` vs `--u-status-warn`) that had been silently degrading the "degraded" pill's own visual distinctiveness since it shipped. Fixed same-session (`296da113`) rather than filed as a follow-up, since it directly undercut this plan's own stated goal.
- **A validate/done gate can surface a genuinely unrelated, load-bearing bug** — `maude kg record-log` (used to persist this close-out's RCA addendum and code-review log into the graph, since `.ai/logs/**` is gitignored) failed ENOENT on every call, for a reason unrelated to this feature: the temp-staging filename never sanitized the `/` that this store's per-repo namespacing puts in every slug. Fixed with a regression test that reproduces the exact failure (`cli/commands/kg.test.mjs`) — worth noting this bug likely means logs recorded via this path before today, in any repo-namespaced kgai store, silently never reached the graph.
- **The whole-repo `pnpm lint`/`pnpm format` gates aren't meaningful mid-close-out in this Syncthing tree** — the working tree carried substantial uncommitted WIP from at least two other concurrent sessions (a figma-import feature expansion, a canvas-render-performance spike) the whole time. Scoped `biome check` on just the touched files was the actually-useful signal; the whole-repo command's failures were 100% attributable to files this session never touched and correctly left alone.
- **Process worked as designed for the small stuff**: the code-simplifier's one finding (hoist a loop-invariant ternary) was genuinely worth taking, applied cleanly, and cost one extra tiny commit rather than complicating the close-out.
