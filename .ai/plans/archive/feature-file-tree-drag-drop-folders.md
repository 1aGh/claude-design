...
- [x] DDR recorded (DDR-199 — renumbered TWICE: 197→198 then 198→199, after two separate concurrent sessions each claimed the number this feature had just taken, caught on two successive merge rebases)
...
- **DDR numbering on a busy shared `main` collided for real, TWICE, not just hypothetically** — a concurrent session claimed DDR-197 for an unrelated cloud-arc decision while this feature was mid-flight (renumbered to 198 on the first merge rebase); by the time the second rebase ran (after fixing an unrelated CI gate + a content-drift gate), *another* concurrent session had claimed 198 too, for yet another unrelated cloud-arc decision (renumbered to 199). `main` was receiving a new commit roughly every few minutes across this feature's `/flow:done` pass — a busy shared trunk isn't a one-off race, it's a standing condition to design for. The project's own numbering-race guidance ("re-check before the closing commit") is correct but insufficient on its own: the actual collision is only visible once `git fetch origin main` runs, which happens at merge/rebase time, not at DDR-authoring time — and a fetch-and-check right before the FIRST rebase doesn't protect against a SECOND collision introduced by whatever lands during the time it takes to resolve conflicts, fix CI, and push again. **Lesson: after resolving a DDR-number rebase conflict, re-check the number is STILL free immediately before the final push — not just once at the start of the merge — since each additional round-trip (a failed CI check, a drift-gate fix) is another window for a fresh collision.**
