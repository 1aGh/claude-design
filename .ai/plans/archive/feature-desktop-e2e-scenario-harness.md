...
- **Running it naostro was the whole value.** Static checks (cargo/tsc/biome/install) were all green, yet the first real `pnpm test:e2e:desktop` surfaced FIVE integration bugs none of them could catch — a published-broken dep, single-instance collision, port collision, first-run wizard, and a slug carrying the designRoot. Lesson for `/plan`: for any harness whose value is "drive the real thing," budget for a real run + a debug loop; don't treat green static gates as done.
- **Early-stage OSS needs version pinning + a registry existence check up front.** `@wdio/tauri-service@1.2.0` (latest) ships a broken import; `1.1.0` works. Task 1's "verify the package actually exists/resolves" caught the existence but not the runtime break — only running it did. Pin exact for churny packages.
- **Shared-tree discipline held.** Executing on a `main` with concurrent uncommitted git-switcher work, the atomic-staging rule worked: 28 of my files committed, the concurrent set untouched. The one unavoidable entanglement (`dist/*` — a single build artifact) was correctly left uncommitted with a note, not force-split.
- **Scope `/done` to repo state.** Full `/validate` (5-platform scenario, a11y, design) is N/A for test infrastructure; `pnpm test` (CLI) is irrelevant here + known-flaky (better-sqlite3 ABI). The meaningful gate set was the harness's own checks + the pilot run. Blindly running the full pipeline would have added noise + read the concurrent diff.
- **Canvas-iframe deep DOM is a real boundary, not a bug.** Made it a soft assertion (cross-origin DDR-054 + render timing); the reliable hard assertion is the top-frame `canvas-frame` + `data-path`, with screenshot evidence for the visual. A future scenario that must prove deep canvas DOM should run with `MAUDE_CANVAS_ORIGIN_SPLIT=0` + generous waits and tighten it.
- **Follow-ups:** dialog/OAuth stubs (debug-only) for first-run + project-switch scenarios; tighten the in-frame content assertion; optional opt-in CI leg (macOS runner + cargo) gated to `workflow_dispatch`.
