...
- **The plan's task list was accurate and the estimates held.** Ten tasks, no pivots, no rewrites. What made that possible was the "Must-Read Files" block naming line ranges: the posture constants, the two polls and the GitPanel call site were found in one pass instead of grepped for. Worth keeping in future plans — it is the cheapest part of the plan doc and it did the most work.
- **`--quick` was the wrong close for this change, and the command said so.** Its own guardrail list names cross-package and security-sensitive work, and this was both. The independent review then found three MEDIUM issues, one of which (a permanent-404 wedge reachable through DiffView's own default `?sha=HEAD`) would have shipped. The lesson is not "never use `--quick`" but that the guardrail list should be read as a gate, not a suggestion — the trimmed pipeline skips build and scenario, not judgement.
- **Source-text pins caught the letter and missed the behaviour.** Two assertions in `cloud-history-posture.test.ts` broke on the very fixes they should have been indifferent to, because they quoted a line rather than asserting a property. The reviewer flagged the same thing independently. When a pin exists to stop a REGRESSION, assert the property; reserve exact-string pins for "this rule must be spelled exactly once", which is what the posture-constant pins legitimately are.
- **A NUL byte lived in committed source for a whole commit because verification greps succeeded.** `IMPOSSIBLE_DOC` held a literal NUL, which made `history.mjs` binary to git — no diffs, no reviewable change, no line-level merge in a Syncthing tree with concurrent sessions. Every post-commit check passed, because `grep` finds strings in a binary file perfectly well. **Add `file <path>` / a NUL scan to the post-commit verification ritual**, not just content greps — CLAUDE.md already says to check content after committing; it should say to check the file is still *text*.
- **The two `/flow:done` review seats returned nothing, twice each, and silence is not a pass.** A replacement general-purpose agent, given the four things I had already verified and asked to confirm-or-refute rather than restate, produced the review that found everything. Adversarial framing plus "here is what I think, prove me wrong" was worth far more than an open-ended audit prompt.
- **Concurrent sessions in this tree are a live hazard, not a footnote.** A local-cell dev harness fabricated 1–16 test failures per run (disjoint sets across identical trees), and another session had uncommitted edits in two files I also touched. Staging by explicit path — never `-A` — and re-running boot-dependent suites alone were both load-bearing, exactly as CLAUDE.md warns.
