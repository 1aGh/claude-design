...
- [x] macOS `.dmg` builds + runs (Task 7) — **`tauri build` produces a working `Maude_0.30.0_aarch64.dmg` (29.5 MB), VERIFIED 2026-06-17.** The bundle ships the dev-server runtime as a resource (`stage-resources.mjs` → `Resources/apps/studio/{dist,client}` + `Resources/plugins/design/templates`), and the sidecar resolves it via `MAUDE_DEV_SERVER_ROOT` (paths.ts override, set by `sidecar.rs` from `resource_dir()`). Bundled-app smoke: `bundled runtime` resolves → sidecar boots first try → `/`, `/_client/client.bundle.js`, `/_canvas-runtime/react.js` all **200**. Real maude icons (from `favicon.svg`) replace the Tauri placeholders. `.github/workflows/build-desktop.yml` runs the same on `v*` tags. **Signing/notarization** still need the `APPLE_*` secrets (your Apple Developer ID) — unsigned dev `.dmg` works now (Gatekeeper: right-click → Open, or `xattr -cr Maude.app`); signed distributable is the phase-32 credential step.
...
- [x] Roadmap regen: `pnpm --filter @maude/site gen:roadmap` run at close (2026-06-17).
...
## Retro (closed 2026-06-17)

- **Plan paths were stale** (pre-DDR-095 `plugins/design/dev-server` refs; `127.0.0.1` vs the real `localhost`; the second `canvasOrigin` port; binary-name vs Tauri target-triple). The founding DDRs (106) corrected them at execute-start. *Lesson:* a phase plan authored before a restructure (DDR-095) needs a path-reality pass before Task 1.
- **Task 7 was the big under-scope.** "Build a `.dmg`" hid the real work: the `bun --compile` binary serves assets from DISK and **builds canvases on-demand**, so the bundle had to ship the `apps/studio` SOURCE tree (canvas-lib + its sibling graph) + a `MAUDE_DEV_SERVER_ROOT` override — not just `dist/`. *Lesson:* for "package X into a bundle" tasks, trace what X reads/executes at runtime before estimating.
- **`tauri dev` ≠ the bundled `.app`.** All three shipped bugs (the `app.restart()` SIGABRT, the stale-`_server.json` white screen, the missing-source canvas failure) appeared ONLY in the real `.app`, never in `tauri dev`. *Lesson:* build + smoke the actual artifact early; don't treat dev-mode green as done.
- **GUI verification ceiling.** The agent can't see or drive the WKWebView window (detached launches exit the WindowServer; agent-browser can't attach to WKWebView). Every interactive bug was caught by the user dogfooding. *Lesson:* native-app phases need the user in the loop for interactive verification — budget for round-trips, ship them a build fast.
- **Adversarial security pass earned its keep — verify findings empirically.** The ethical-hacker's headline F1 (Bun build-time macro RCE) was a sharp hypothesis; an empirical test (a macro canvas → 500, no execution; the pass-1 oxc transpile blocks it) disproved it, avoiding a speculative `macros:false` of unverified validity, while F3/F4 were real cheap hardenings worth landing. *Lesson:* run the security pass on new native surface, and test adversarial claims before fixing.
