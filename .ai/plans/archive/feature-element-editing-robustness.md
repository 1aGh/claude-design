...
- `.ai/archive/decisions/DDR-139-reused-component-reorder-and-phase-12.1-followups.md` (read in full — canonical shared-instance description + occurrence-index solution + the `.map()` caveat), `DDR-019-canvas-tsx-format.md` (positional cd-id), `DDR-138-jsx-node-move-reorder-and-id-resettle.md` (the move/reorder + id-resettle model).
...
- **In-repo — `.ai/archive/decisions/DDR-104-css-panel-ux-model.md`** — Why: **this plan's Stage B reverses DDR-104 decision §3's OUT-list** (`position`, `top/right/bottom/left`, `transform`, `text-transform`, `object-fit`, `aspect-ratio` were explicitly deferred). Read its §3 + Addendum to reuse the vocabulary/provenance rules and to write the superseding DDR (Task G1) correctly.
...
#### ✅ Task I3: CREATE the `insert-element` source op + route + palette — DONE 2026-07-07 (engine + route + shell + context-menu Div/Text/Image + tool-palette "+ Element" — v1 scope: appends to the active artboard, not full click-to-place)
...
- **Gotcha**: DDR numbering races on shared `main` (memory `project_ddr_numbering_races_on_shared_main`) — re-check `.ai/archive/decisions/` immediately before numbering; the tentative next number is **DDR-152** (current max is 151). This may warrant **two** DDRs (the knob/scope UX decision and the structural-edit/undo decision) if one grows unwieldy.
...
- [x] `/flow:validate` passes overall — run via `/flow:done` 2026-07-08:
  - [x] Static (types, lint, format) — `bun tsc --noEmit` clean (6 pre-existing DDR-026-baseline errors only), `pnpm lint`/`pnpm format` clean, `maude doctor` 0 schema errors
  - [x] Tests — CLI **178/178**, dev-server **2174 pass / 1 skip / 1 fail** (pre-existing flaky `git-api.test.ts`, clean in isolation)
  - [x] Build — site build OK; client bundle rebuilt `--release`; parity/tarball/tokens/site-content gates all green (one pre-existing site-stats drift from an earlier feature's docs page found + fixed)
  - [x] `scenario-runner`: 0 blockers. Authored + ran all 3 Stage-G scenarios (`element-editing-resize-and-position`, `specimen-and-media-editing`, `structural-and-scope`) on `web-desktop`; parity N/A (single-platform, documented). A few sub-steps (deep-select, Alt-hover measure, artboard-frame-select, instance-local-move) landed as "partial" — confirmed harness limits of synthetic pointer dispatch, not product defects (each has an independent unit test or a passing sibling step). Reports: `.ai/scenarios/<slug>/2026-07-08-1147/report.md`.
  - [x] `design-system-guard`: 0 blockers. New align/distribute/tidy icons verified byte-identical to the `MultiArtboardToolbar` source; tokens, reduced-motion coverage, and the two precedented hardcoded overlay colors (measurement red, gap pink) all conform. One minor cosmetic CSS suggestion (non-blocking).
  - [x] `a11y-auditor`: 0 WCAG 2.1 AA blockers. Live axe-core scan clean on the toolbar/context-menu/resize-handles/measure-overlay; 2 non-blocking warnings (one is a pre-existing shared dropdown-menu pattern the new View-menu item inherits, not new).
- [x] `/flow:validate-security`: 0 blockers at/above `medium` (both defender + adversarial passes). This diff adds no new HTTP route — align/distribute/tidy-up reuses the existing `reposition-request`/`/_api/edit-css` lane, confirmed pinned to `activePath` with no cross-canvas/cross-element write possible; the `editStringAttr` src-expression refusal is a net hardening, correctly scoped. Report: `.ai/logs/security-reviews/main-20260708T095647Z.md`.
...
- **What worked:** the four-agent research read up front (selection/toolbar, camera/move/resize, inspector/knobs, specimen/media) meant almost every task in Stages A–M had exact file:line anchors before implementation started — very little architectural discovery needed mid-execution. Reusing existing write lanes everywhere (align/distribute/tidy-up rides the same `reposition-request` message an element drag already uses; structural delete/insert/artboard-insert reuse the reorder-log whole-file-snapshot undo instead of new command classes) kept the security surface flat despite the huge feature surface — both security-auditor and ethical-hacker's `/flow:done` passes came back clean on the first try because there was nothing new to attack.
- **What worked, part 2:** running a G2 test-coverage *audit* (not just "add more tests") caught a real bug — `editStringAttr` was silently discarding a `src={expr}` JS-expression binding instead of refusing, contradicting its own code comment. The bug had been sitting in already-shipped, already-dogfooded code from a prior session; it surfaced only because the audit asked "is this actually tested" rather than assuming coverage existed.
- **What didn't work / friction:** the dev-server's dev/release bundle self-heal bit again — booting the server for live scenario/a11y verification regenerated unminified dev bundles on top of the clean release-minified commit, and it wasn't caught until the final `git status` review before commit. CLAUDE.md already documents this trap; the fix is to always re-run the `--release` build as the LAST step before `git status`/commit, not just once earlier in the session.
- **What didn't work, part 2:** the scenario-runner subagent (the heaviest of the five `/flow:validate` fan-out agents — authoring 3 scenarios AND driving a live browser) took roughly 10x longer than the other four combined, with several silent-looking stretches that turned out to be real progress (not stuck) once checked via the shared task list. A future `/flow:validate` fan-out should set an expectation up front that scenario-runner is the long pole, so a waiting session doesn't misread a long quiet stretch as a hang.
- **What to change next time:** author the scenario spec.md files (or at least a throwaway fixture canvas) *during* implementation, not deferred to close-out — by the time `/flow:done` ran, nobody remembered the exact DOM shape needed (absolutely-positioned overflow box, a 3×-used shared component, etc.) without re-deriving it from the plan text, and the scenario-runner had to build a fixture from scratch under time pressure. A `.design/ui/*.tsx` scratch fixture checked in early (even as a WIP canvas) would have made this close-out gate much cheaper.
- **Concurrency note:** `main` is being actively worked by multiple concurrent sessions (an unrelated `/flow:done --quick` feature and Intel Mac desktop support both landed mid-close-out). No conflicts this time — `STATE.md`/`whats-new.json` prepends from other sessions were compatible with mine — but it's worth remembering commits here are truly interleaved in real time, not just "possible drift," when reviewing `git log`/`git status` before staging.
