...
- **The Task 6 fork (lossy approximation vs. full DOM-parser fidelity) was worth surfacing explicitly** rather than picking one unilaterally under scope pressure — the user's answer ("do it properly") revealed a real priority (correctness over speed) that shaped how much later effort was worth spending hardening that exact code path. Keep raising genuine cost/quality forks via `AskUserQuestion` rather than silently defaulting to the cheaper option.
- **Live verification during `/execute` caught a real bug** (chained `move`+`set-text` on a pre-existing id clobbering) that unit tests alone, written from the same mental model as the implementation, would likely have missed — the discipline of actually running the built thing against a real server, not just asserting on its output, paid for itself again.
- **The security fan-out at `/done` was where the heaviest, highest-value findings landed** — not because `/execute`'s per-task verification was sloppy, but because Task 6's happy-dom integration created a genuinely new class of risk (privileged-process parsing of peer-writable content) that a feature-correctness lens doesn't reliably catch. Two full rounds were needed: the first fixed 2 bugs and revealed a packaging assumption was wrong (a "fix" broke the parse, caught only by re-running the existing tests); the second round, spawned specifically to re-verify the first round's fixes, found 3 MORE concrete bugs the first round's own reviewers didn't have reason to look for (uncapped `connections[]`/`edges[]`, no read-time size cap, a real npm-packaging gap). **Takeaway for future features touching a new privileged-parsing surface: budget for at least two security re-verification rounds, not one — the first round tends to find the obvious shape of a problem; a second, freed from re-deriving context, finds the adjacent gaps the first missed.**
- **A shared, actively-committing `main` tree cost real time** (repeated `dist/*` dev-form re-clobbering from test runs, one genuine hunk-interleaving episode in `canvas-lib.tsx` that self-resolved when the other session committed). None of it was harmful, but tracking "is this diff mine" added meaningful overhead across a long session. No process change recommended — this is the accepted cost of dogfooding flow on a repo with concurrent human+agent activity — but future long `/execute`→`/done` runs on this repo should expect it and budget time for the bookkeeping.
- **One item punted, tracked, not resolved:** the happy-dom production-packaging gap (nested workspace `package.json` doesn't ship in the npm tarball) got a graceful-degrade fix, not a root-cause fix. A proper fix (routing `annotate`/`canvas-rects`/`read-annotations` through the compiled-binary dependency-embedding path, or another distribution mechanism) is real follow-up work, documented in DDR-151's Consequences — don't let it silently drop.
