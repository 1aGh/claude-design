...
## Retro (Phase 2 close, 2026-08-24)

- **The fidelity gaps were invisible to same-engine fixtures — every one needed a real cloud-shaped repro.** Layer tests (capture-core vs a synthetic fixture, lane resolution as data, validBody over curl) were all green while a live export shipped the artboard's title bar, dropped every photo, lost fonts, filters and the Sash stripe, and had no assets in the HTML. The turning point each round was standing up the `~/maude-smoke` cell (real hub + render + canvas origin) with the actual alligators project and reading the *bytes*. Lesson for `/plan`: for any lane/host-split feature, a "workspace-shaped repro on a real project" is a T1 deliverable, not a nice-to-have.
- **dom-to-svg is a partial CSS renderer, and its gaps are silent.** Image inlining never worked (writes `xlink:href`, reads `href.baseVal`), `object-fit` ignored, `filter`/`repeating-linear-gradient`/`clip-path` dropped — each caught and console.error'd internally, so nothing surfaced. The capture spine now owns polyfills for all of them. Any future "it looks different in the export" is probably one more of these; check dom-to-svg's element.js/inline.js FIRST.
- **The cell→worker boundary re-introduced the same class of bug three times: a guard that reads the checkout no-ops on the worker (which has none) and passes vacuously.** Scope targets (DDR-230), print props, and the `<Audio>` pre-flight all failed this way — the worker did exactly what each guard existed to prevent. Codified as `maude/export-checkout-inputs-ship-in-job`: anything derived from the checkout is resolved by the cell and shipped in the job as data. A NEW export input read from disk in an adapter should trip a review flag.
- **Async `_active.json` vs the live dialog** — a job submitted right after a tab switch rendered the previous canvas with the new canvas's artboardId. The dialog knows its own file; it now stamps `canvasFile`. Generalizable: any submit that resolves against server-side "current" state races the UI; carry the identity in the request.
- **Parallel test runs on this box fabricate failures** (memory `maude-parallel-test-runs-contaminate`) bit twice during this close — a 10/12 "failure" that was 25/25 clean on an idle machine. Ran every gate serially. The `export-lanes` CI job (now required) is the durable guard.
- **Two of the three original plan diagnoses were wrong** (the token was a red herring; OOM was never the video fault — CSP was). Repro-first held; the plan's hypotheses were treated as leads, not conclusions.
- **The /flow:done security fan-out earned its place.** The defender + attacker pass on the finished diff found what the feature tests could not: the png shim was hardened against `data-dc-screen` path traversal and its two siblings (svg, html) were left raw; the self-contained HTML artifact executed tenant script on open; a viewer's "record my browser export" was a byte-eviction delete primitive for other members' artifacts; and the cell→worker options bag was unchecked passthrough. All fixed in-diff with fail-first tests. The signal in most cases was an *inconsistency* the "unify all three lanes" goal created (one lane hardened, siblings not) — a review that only diffs changed lines misses it.
