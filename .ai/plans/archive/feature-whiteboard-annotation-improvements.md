...
- **Live dogfood testing (by the user, mid-execution) surfaced real bugs a solo agent pass wouldn't have caught** — no blinking caret, double-click select-all instead of click-to-place, hover cursor, keyboard-shortcut leak while typing, Text-tool not editing in place. All six requests plus these six extra fixes shipped in one `/done`. Next time: budget for a "have the user try it live" checkpoint explicitly in the plan for any UI-heavy feature, rather than treating it as an unplanned interruption.
- **Identity/provenance fields got less adversarial scrutiny than content fields, and the security fan-out caught it precisely because it ran independently.** `authorName`/`authorId` (DDR-155) shipped with a genuine `esc()`-vs-`escAttr()` regression and a missing entity-decode step in the CLI reader — both invisible to normal code review because "it's just a name" doesn't *read* as attacker-controlled the way "note text" or "url" does. The regression test added during the fix (`authorName containing '>' round-trips intact`) is now a permanent guard against this recurring. Lesson for future "author/identity"-shaped fields: hold them to the exact same escaping/decoding bar as content fields from the start, not after a security pass finds the gap.
- **A pre-existing gap (missing `e.source` check in `annotations-layer.tsx`'s postMessage listener) got worse, not better, by being extended rather than fixed** — the new `insert-sticker` case was added to an already-gapped listener without anyone noticing the sibling file (`canvas-shell.tsx`) had already closed the same gap for its own listener. Lesson: when adding a new case to an existing cross-frame message handler, diff it against the equivalent handler in sibling files first — inconsistency between them is itself a signal.
- **Running `bun test` (or booting the source dev server) regenerates the committed `dist/*` bundles to an unminified ~8MB dev form** — this bit the session repeatedly (4+ times) despite being a known pattern from CLAUDE.md. Keeping a manual `/tmp/dist-release-backup/` and diffing after every test run worked, but is fragile and easy to forget. Possible follow-up: a pre-commit or CI check that fails loud if `dist/client.bundle.js` exceeds the `.min-sizes.json`-style floor, so a forgotten restore can't silently ship.
- **Sticker sourcing (4 manually-curated Figma Community packs) was the single most manual step in the whole feature** — no API, auth-gated `.fig` exports, non-descriptive filenames requiring a hand-built contact sheet to caption. Worked fine as a one-shot cost, but a 5th pack later follows the exact same recipe (`build-stickers.mjs` + `write-sticker-manifests.mjs`), which is worth keeping in mind if "add more sticker packs" comes up again — it's not a quick ask.
- **The `--quick` code-review fan-out (parallel security-auditor + ethical-hacker + code-simplifier) stalled once on infra (600s watchdog, all three) and needed a full retry** — worth knowing this can happen on a large multi-file diff; the retry with a slightly tighter "work efficiently, don't over-analyze clean files" instruction succeeded cleanly on the second attempt.
