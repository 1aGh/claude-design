...
- [x] No DDR-worthy decision left unrecorded — recorded as [DDR-165](../decisions/DDR-165-annotation-commit-reconciliation-baseline-not-presence.md) at `/flow:done` time. Live dogfooding after the initial commit surfaced two further bugs in the same reconciliation territory (the chain's idle-reset trusting a stale-but-present snapshot value, and a local-commit/collab-echo fold-back helper reverting deletes) — DDR-165 covers the full, final mechanism as shipped, not just the plan's original scope.
...
- **What worked:** the functional-updater + Promise-chained-accumulator pattern (`media-commit-chain.ts`, mirroring `client/app.jsx`'s `structuralWrite`) correctly diagnosed and fixed the *reported* race on the first pass, and the deterministic tick-based unit tests (no real timers) made the chain's behavior genuinely testable without a browser. Live agent-browser verification (not just unit tests) against a real dev server was what actually caught the deeper bugs — pure unit tests with synthetic interleaving didn't force the exact failure shape until a test was written to specifically match a live-observed symptom.
- **What didn't:** this bug class recurred THREE times across one feature — the chain's idle-reset trusting a stale-but-present snapshot value (caught by live dogfooding, fixed in `6724d700`), a fold-back helper reverting deletes (caught by a real user bug report five minutes after declaring the previous fix done, fixed in `6c5e46bf`), and a delete-during-upload resurrection at a third call site with the identical root ambiguity (caught only by `/flow:done`'s adversarial code-review pass, not by any live test run). Each fix addressed the SPECIFIC site found without a systematic sweep for the same shape elsewhere in the file, so the same mistake ("id missing from the other array ⇒ assume not-yet-arrived, never assume deleted") shipped three separate times before code review caught the last instance.
- **What to change next time:** when a reconciliation/fold-back helper is introduced to fix a concurrency bug, immediately grep the surrounding file for every OTHER call site with the same shape (`X.filter(id not in Y)`-style presence checks feeding a "re-add it" branch) and audit each one for the same ambiguity, rather than fixing reactively one live-bug-report at a time. Also: run the adversarial code-review pass (or at least a deliberate "how would I abuse this race" self-review) immediately after the FIRST concurrency fix, not only at `/flow:done` close-out — it would have caught the resurrection bug two rounds earlier, before a second live "sometimes it's still broken" report was needed.
- **Process note:** `/flow:execute` → live dogfood → bug report → fix → close is a real, working loop for this kind of bug, but it cost three iteration rounds here. A dedicated "concurrency/race regression" checklist item in the design-system or code-review agents (specifically: "for every place that folds one array against another by id, verify delete is distinguishable from not-yet-arrived") could shortcut this the next time a similar accumulator/reconciliation primitive is introduced.
