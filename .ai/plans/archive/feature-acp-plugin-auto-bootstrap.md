...
- **The go/no-go spike paid off — and a live isolation trick beat a black-box test.** Injecting a *uniquely-named throwaway* plugin (not `design`) cleanly proved the `_meta.claudeCode.options.plugins` mechanism against the real adapter+`claude` even though the author already had `design@maude` installed. Reusable pattern: to test an injection mechanism when the real payload is already present, inject a marker that can't pre-exist.
- **Native-app verification has a hard floor, but more is testable than it looks.** The plugin injection + readiness both got *live* proof without the packaged `.app` — a pristine `CLAUDE_CONFIG_DIR` + the `/_api/preflight` endpoint from a running `tauri dev` exercised the whole server-side chain. The pristine-config sandbox also surfaced a real gotcha: an empty `CLAUDE_CONFIG_DIR` = logged-out `claude`, so the chat *hangs* on a real prompt (only `session/new`/catalogue works logged-out). "Fresh user" demos need the real config with the plugin registry stripped, not a blank config.
- **Shared-`main` concurrency is the dominant execution hazard here.** A parallel session repeatedly reset/re-committed `main`, twice sweeping staged files into its commits and once trimming this feature to design-only. Mitigations that worked: stage *explicit* files (never `-A`), commit *atomically* (stage+commit in one shell call to shrink the race window), and verify `git cat-file -e HEAD:<file>` after every commit. `/execute` + `/done` on a live shared branch should assume the index can change under them.
- **The security fan-out found the real teeth, and they were in the *composition*.** DDR-143 alone was clean; the follow-up DDR-144 (screenshot engine) + DDR-143 composed into an "open-a-repo → get-owned" confused-deputy chain (settingSources breadth × auto-approve × a new exec sink). Lesson for `/plan`: when a feature adds an *execution sink* or *outbound fetch* to an already-auto-approving surface, budget the adversarial pass up front — the deferred "guard #6 follow-up" turned out to be a HIGH, not a nicety.
- **Scope drift is fine when it's the user's call.** `/flow` trimmed `/flow:*` auto-load to design-only mid-flight (a parallel session); building the resolver as a pure, per-plugin gated function made that a one-line change, not a rework. Small, testable gate functions absorb scope changes cheaply.
