...
- [ ] `/flow:utils-verify` passes after each task — NOT run per-task this session (verified instead via targeted `bun build` parse checks + a direct-screenshot render of the new specimen against the live dev server; a full agent-browser interactive pass on the CSS/Photo panels — drag, arrow-keys, popover — is still recommended before `/done`)
...
- [x] Scrub is on the icon/label, click select-all + click-again-caret works, arrow-key stepping works (Image #1 + #2 satisfied) — for `num()`'s NumberField fields. `side()`/`inset()`/`corner()` (36×24px box-model cells) deliberately keep whole-cell scrub (no room for a separate handle — the Figma/Webflow convention for cells this small) but gained select-all + arrow-keys via the same shared `makeScrubHandler` engine.
- [x] No row overflows at 260px or 304px (Image #3 satisfied) — border row: `fixedUnit` drops the unit-select + `flex-wrap` fallback
- [x] Variables popover anchors to its trigger at any scroll/zoom (Image #4 satisfied) — `place()` re-runs on scroll/resize instead of dismissing, plus a rect-diff compensation pass that cancels any transformed-ancestor offset regardless of cause
- [x] Photo ADJUSTMENTS are real linked sliders; DUOTONE + Pattern fit at default width (Image #5 satisfied)
- [ ] New `components-inspector-controls` specimen renders styled; `/design:smoke` + completeness-critic pass — specimen verified styled via a direct screenshot against the live dev server (dark theme confirmed; light-theme force-override didn't retint, matching other specimens' known behavior under the smoke tool's theme-override eval, not a regression). The repo's `/design:smoke --changed-only` run itself came back 39/64 UNSTYLED — but identically across every `system/maude/preview/*` specimen including untouched files (`logo.tsx`, `radii.tsx`, `components-inputs.tsx`), and it was talking to port 4405 per its report header, which `/_health` shows is serving a DIFFERENT project (`sandbox-test`) — a pre-existing stale `.design/_server.json` / multi-server environment issue in this shared tree, not a regression from this plan. `design-system-completeness-critic` not run this session.
- [x] Committed client bundle rebuilt `--release` — built (`apps/studio/dist/client.bundle.js` 1.79MB / `styles.css` 231KB, both release-minified); not yet committed to git
- [ ] `a11y-auditor` + `design-system-guard`: 0 blockers — not run this session
- [x] Phase-2 dual-mode captured (this doc) but not started
- [ ] Any DDR-worthy decision recorded — two candidates flagged but not written up: (1) box-model cells keep whole-cell scrub rather than an icon handle (documented inline instead), (2) TokenPopover fix uses rect-diff compensation rather than hunting the exact transformed ancestor
