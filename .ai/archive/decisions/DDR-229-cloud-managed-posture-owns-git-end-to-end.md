...
### What the security re-review added (2026-08-18, verdict PASS WITH SUGGESTIONS)

Two properties are **accepted deliberately** rather than fixed, and are recorded here because silence about them would read as oversight:

- **A `viewer` can read the full commit log and any admitted design-root blob at any commit.** Both routes are GET, so the `readOnly` gate is moot, and neither checks `role`. This is a real widening of what a viewer sees versus the live tree — a viewer now sees author, subject and date for every commit, repo-wide when their token is unscoped. It is intended: History is the surface DDR-218 kept *because* looking back at what the cell saved is the point, and withholding it from the role whose whole permission is "look" would be incoherent. Recorded so the next reader does not mistake it for an unchecked gate.
- **A peer who can write the committed `config.json` can re-point History at another hub the same user is credentialed to.** `normalizeUrl` rejects embedded userinfo and the credential lookup is keyed by the normalized address, so a peer cannot name an address they control and receive a token — the token for hub B goes to hub B. The residual is **misattribution, not exfiltration**: History would display hub B's commits under project A's header (and the `hubHost` fallback would name B). Accepted; the corroboration rule (`credentialed`) already bounds it to hubs the user has genuinely signed into.

Three findings were **fixed before the feature closed** — a transient cloud failure or a mutable ref being remembered as a permanent absence, an uncapped response from the cell, and UTF-8 corruption of `git show` output across pipe-chunk boundaries. The last was latent in both git runners and made live by this change routing whole canvas bodies through them; it is fixed in both, because the premise of the shared log module is that a cloud row and a local row cannot differ in shape, and "one of the two mangles diacritics" is exactly that kind of difference.
