...
**Status:** done — **feature-sync-burn-down-and-shared-doc** Tasks 1–2 closed 2026-08-18 under a user PRECONDITION OVERRIDE (branch `feat/sync-journal-file-plane`, archived). The burn-down (Increment 5) deleted the old transfer engines — `asset-pull.ts`, `asset-sweep.ts`, `asset-push-worker.ts`, `pushOneAsset`, the fast lanes — leaving `asset-push.ts` as the in-process legacy compat client, lane chosen once per boot by `decidePushLane` (a journal hub never sees it). The hub's directory-walking sweeper became a journal-driven `createWriteBehind` over `mirrored_at_ms` covering EVERY plane class (closes F-6/B2), with `hydrateFiles` as its restore half; legacy PUT routes delegate to the one file door. Increment 7 flipped `MAUDE_SHARED_DOC` default ON (`sharedDocEnabled`, `=0` is the rollback), nothing deleted. **Core delta −801 lines.** Increment 8 (delete `agent.ts` + the two-doc relay) deliberately held for next release so the shared-doc flip keeps its config-flip rollback.

Two-seat security review at the close: **defender PASS ×2 (0 crit/0 high)**, attacker NEEDS-FIXES reassessed to **PASS with ONE tracked medium** — F1 (write-behind symlink read) fixed at the close, F3 filed below-floor, and **F2 (the DDR-064 A7 consent notice is a `console.warn` a terminal-free desktop user never sees) is a BINDING follow-up before Increment 8** — a GUI consent surface, DDR-064's to own. Not a data-exposure regression (hub trust is consented at link time; the flip changes the doc mechanism, not the recipient).

> ⚠ **Closed under `/flow:done --quick` + a precondition override.** A full `/flow:validate` (build, cross-platform scenario, a11y, design-system) is MANDATORY before this branch merges to main. Also deferred to the release, not the close: real-tree byte parity (alligators → mirror), desktop self-containment against a built `.app`, the old-hub LIVE e2e, and the fleet poke drill. Version is still 0.60.7 — nothing here has shipped.
