...
**Status:** in progress — **feature-sync-journal-file-plane**, Increments 0–4.5 and 6 closed 2026-08-18 (branch `feat/sync-journal-file-plane`). Seven sync lanes are now two: Plane A (canvas CRDT docs) untouched, Plane B one journal-driven file lane — hub-ordered append-only `file_journal`, a payload-free poke over the existing Hocuspocus stateless channel, a per-hub ledger, and one pure three-way `decideFile` table. **DDR-226** (architecture), **DDR-227** (git-as-transport rejected), **DDR-228** (two ownership modes).

**What shipped in the second half of the day.** Increment 4 closed the security gate: all 25 findings from the two-seat review are fixed, each with a test verified to fail against the unfixed code. Three of them turned out NOT to be behind the flag they were filed under — the ctl channel's awareness was an authenticated cross-scope broadcast bus, a missing R2 tail did not rotate the epoch, and the hydrate journal hook threw on every call because it referenced two names from another scope. `linkedHub.syncFiles` then flipped **ON**: the whole `.design/` folder mirrors both ways. Increment 6 made deletion real — a tombstone is a journal row with the same CAS a write has, so an edit that raced a delete wins at the door; nothing is unlinked, losers go to `_trash/` on both ends; and breakers hold in BOTH directions past 10 files or 25% of what a machine tracks. Increment 4.5 ended the hybrid: a project is repo-owned or hub-owned, `adopt`/`detach` switch between them without moving a byte, and Syncthing gets its own warning because it does not read `.gitignore`.

**Deviations, taken knowingly:** `propagateDeletes` ships ON rather than OFF-for-one-release (Open decision 1), which makes the breakers the only protection — they were built for that. And F2/F3 keeps the SHIPPED classifier over the synthesis line: defaulting empty-tree in-group css to canvas-owned is what lost `brand.css` and `_layout.css`, so the tests pin convergence instead of the default.

**Next:** `.ai/plans/feature-sync-burn-down-and-shared-doc.md` — Increment 5 (burn-down, ≈ −900 core LOC) and Increments 7–8 (shared-doc epilogue). Split out because both DELETE working code and the arc's own rule is that nothing goes until its replacement has soaked a **released** version; Task 7 also gates its poll relaxation on a measured poke-miss figure.
**Active plan:** `.ai/plans/feature-sync-journal-file-plane.md` (closing; Tasks 0–8 done, 7+9 moved out)
**Active task:** —

**Carried debt:** the doručenka's per-file rows reach `_sync.json` but the Sync panel still renders aggregates only; the `sync-doruceka-panel` scenario was never written; adopt/detach is CLI-only, so DDR-177's terminal-free desktop dialog is unbuilt; new-machine clone→link→pull parity is unit-covered rather than run on a second machine. The studio suite is green in isolation but fabricates 1–5 port-bind failures per run under machine load.
