...
**Status:** done — **feature-sync-file-plane** closed 2026-08-14 (commit `18f7c290`). Sync's unit was a canvas, so a fresh link delivered every canvas and lost 103 other files (the design system). Replaced the growing per-file-kind taxonomy with **two planes + one positive classifier** (studio↔hub parity-pinned): Plane A (canvas CRDT docs) untouched; Plane B is a manifest-driven file plane (hub `GET /api/files` + read route `GET /_project-file/<rel>`), the peer re-classifies every entry against its own tree before writing (never folder trust). `config.json` + runtime state never travel; code modules only from an owner-vouched/loopback hub; LWW conflicts park the loser in `_trash/`. Behind `linkedHub.syncFiles` **default OFF**. Acceptance: **216/216 byte-for-byte** on the real alligators tree. **Two-seat security review ran** — one above-floor blocker (read-route leaf-symlink follow → served `never`-class bytes) fixed + regression-tested; every binding attacker chain closed; residual findings gated by flag-off and recorded as a **mandatory pre-flip security gate** (`kg maude/sync-file-plane-security-verdict`). Full `test:dev-server` browser suite deferred to a quiet machine (load 100–200 during the close); sync/hub/cli portions green.

**Next cycle (gated):** flipping `syncFiles` default ON re-runs `/flow:validate-security` as a HARD gate closing F1 (agent-reads-synced-docs-as-spec / trifecta), F2/F3 (fresh-link disjointness), F4 (hub-mtime overwrite), F6 (byte budget) FIRST; then retire the reference-derived asset scan + author the What's New entry.
**Active plan:** —
**Active task:** —
