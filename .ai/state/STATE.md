...
**Status:** in progress — **feature-sync-journal-file-plane**, Increments 0–3 closed 2026-08-18 (branch `feat/sync-journal-file-plane`, 31 commits). Seven sync lanes collapse toward two: Plane A (canvas CRDT docs) untouched, Plane B is now ONE journal-driven file lane — hub-ordered append-only `file_journal` + epoch + per-peer cursors, a payload-free **poke** over the existing Hocuspocus stateless channel, a desktop per-hub **ledger** (ancestor store + stat cache), and one pure three-way `decideFile` table. Architecture in **DDR-226**; git-as-transport developed and rejected in **DDR-227**; two-mode design ownership in **DDR-228**. The Increment-2 poke killed the live watcher-gap bug — a hub-door write now heals an open cloud tab without a reload.

**What the two-sided E2E found that the increments could not** (`scripts/dev/sync-e2e.mjs`, every scenario run BOTH ways): the nudge had shipped with **no caller** (seconds one direction, up to 15 minutes the other, every unit test green); a canvas created on a peer arrived with **no title**, and fixing it exposed a meta lane that two peers could poison with duplicate whole-value writes (nine live documents); and one deleted canvas **wedged the cloud's autocommit permanently** while `/health` answered 200 and every file was present. Two findings recorded, not fixed (cloud git history covers canvases not the project; a peer-moved canvas never reaches a brand-new machine — entangled with Increment 6).

**Security at the close:** two-seat review over the 87-file diff, both seats **NEEDS FIXES** (9 + 16 findings). Everything reachable **without** the `syncFiles` flag was fixed here with tests verified to fail against the unfixed code — write-door token scope, the owner gate judged on the lexical rather than the real landing path, the check-then-act CAS, and `movedTo` as an uncapped peer-controlled deletion trigger — plus the two cheapest pre-flip items (the budget charged the hub's claimed size; the body was buffered before the cap ran). Verdict in the graph: `kg search "sync v2 Increments 0–3"`.

**Next (Task 5 = Increment 4):** the F1–F6 hard gate, which now carries a **named worklist** from that review (F-3 receiver-side symlinked-parent containment, F-4/A7 the poke+reanchor cooldown, A5/A6/A8/A9/A12/A13/A14/A15, F-6..F-9 — A7's ctl half, A12 and A13 are live, not flag-gated). `syncFiles` cannot default ON before those close. Then Tasks 6–9: adopt/detach (DDR-228), burn-down, deletion propagation, shared-doc epilogue.
**Active plan:** `.ai/plans/feature-sync-journal-file-plane.md` (Tasks 0–4.9 done; 5–9 open)
**Active task:** Task 5 — Increment 4, the F1–F6 hard gate

**Carried debt from this close:** the `sync-doruceka-panel` scenario was never written, and the doručenka's per-file rows reach `_sync.json` but the Sync panel still renders aggregates only. The full `test:dev-server` suite is green in isolation; under machine load it fabricates port-bind failures (2–5 per run, a different set each time).
