...
The plan (`feature-4-canvas-editing-figma-parity.md`) paired this selection model with two further tracks: an incremental Figma-grade **layers** upgrade and a **convert-to-absolute** context action (the latter its own DDR-188). This session shipped the selection model (above) in full, plus the layers panel's highest-value fix — **synthetic group rows for unstamped wrappers** (the "some elements don't show up / the tree is flat" bug) and **auto-reveal** of the selected row. The layers **component-instance colouring / locked state / inline rename** and the whole **convert-to-absolute** capability (DDR-188) are deferred as scoped follow-ups (see the plan's Retro); the non-goal comments convert would reverse are therefore left intact.
