...
> **Addendum — 2026-07-03 (same-day dogfood): download-first tile imagery.** Tile images are **downloaded at compose time** into `<designRoot>/assets/moodboard/<ds>/` (`curl -fsL` with a browser UA) and referenced by **local path**; an external hotlink is used only when the download fails, with the `onError` labeled-scrap fallback as the last resort. Evidence: in the first live dogfood, valid wikimedia URLs that fetched fine via curl rendered in the headless screenshot but **failed in the user's browser** — hotlink reliability is browser-context-dependent (UA/referer policies, rate limits), and under split-origin CSP (`img-src 'self'`) hotlinks are blocked outright while local files pass. Download-first makes the direction gate deterministic and keeps the persistent canvas (DDR-136) actually persistent — hotlinks rot. Supersedes DDR-080's "external-image reliability is an accepted risk" posture for the default path (the risk now applies only to the residual hotlink fallback). The same dogfood also hardened the **density floor** (user feedback: tiles read "empty"; a professional board is *plnotučný*): ≥ ⅔ scrap coverage, no bare region > ~12 %, ~12–18 scraps per miniature tile / 25–40 on the expanded board, thin imagery compensated by same-image multi-crops + type crops + material swatches + UI micro-fragments — plus a mandatory **pre-gate density self-check** in Step 2 (sparse ⇒ add scraps + re-shoot before asking). Instruments: `_bootstrap.md` Stage-3 seeding bullet, image-density layer 1 + density-floor bullet, CSP note, Step 2 self-check, escalation self-harvest bullet.
