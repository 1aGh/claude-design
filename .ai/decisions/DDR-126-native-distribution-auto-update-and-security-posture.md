...
- **Accepted / tracked (not blockers for v1):**
  ...
  - **F7 — single feed endpoint, no failover.** Losing/taking over `maude.iagh.cz` is a **denial-of-patch** (availability) only — a feed attacker cannot push malicious updates without the signing key. Follow-up: add a static-`latest.json`-on-GitHub-Releases second endpoint as failover.
