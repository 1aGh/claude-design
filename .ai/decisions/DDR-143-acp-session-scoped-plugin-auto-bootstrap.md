...
6. **`settingSources` breadth (pre-existing leak note):** the adapter defaults to `["user","project","local"]`, so the spawned `claude` already reads the served downstream repo's `.claude/`. This feature does not widen that; narrowing it for the bootstrap case is a tracked follow-up, not a regression introduced here.
