...
| --- | --- | --- |
| `.ai/archive/decisions/*.md` (190) | **full body** + tags + `SUPERSEDES`/`EXTENDS`/`REFERENCES` edges | committed. Still the **authoring surface** (kgai has no editor — you write the file, then ingest) and 622 references across the repo point at these paths. |
| `.ai/archive/logs/**` (120) | **full body** + date + `EVIDENCE_FOR` edges to cited DDRs | **gitignored** — the graph is their only inheritable copy. |
...
**Runtime-state taxonomy is canonical in [DDR-115](.ai/archive/decisions/DDR-115-per-user-camera-split-and-runtime-state-taxonomy.md).** What's IGNORED (per-machine/per-user runtime: `_canvas-state/` incl. the `<slug>.view.json` camera, `_state/`, `_chat/`, `_comments/`, `_untrusted/`, `_history/`, `_trash/`, `_draw/`, `_smoke/`, `_server.*`, `_active`/`_sync`/`_preflight`/`_locator`/`_export-history.json`) vs VERSIONED (`*.tsx`, `*.meta.json`, **`*.annotations.svg`**, `system/**`, `config.json`) must agree across THREE lists — `apps/studio/git/service.ts` `isMaudeRuntimeState`, `cli/lib/gitignore-block.mjs`, and the repo `.gitignore`. They drifted once; when adding a new `_*` runtime path, update all three together. A canvas's pan/zoom (`viewport`) is NEVER in the versioned `.meta.json` — it lives in `_canvas-state/<slug>.view.json` (GET merges it back, PATCH splits it out; `last_modified` is stamped only on a layout change).
